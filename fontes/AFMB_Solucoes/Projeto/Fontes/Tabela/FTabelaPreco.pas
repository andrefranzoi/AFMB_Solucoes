...
begin
   ...
   Try
     ...
     begin
        ...
        QryListaProduto.FieldByName('CODIGO').AsInteger    :=db_TabelaPreco.FieldByName('CODIGO').AsInteger;
        QryListaProduto.FieldByName('REFERENCIA').AsString :=db_ReferenciasImportar.FieldByName('REFERENCIA').AsString;
        QryListaProduto.FieldByName('DESCRICAO').AsString  :=db_ReferenciasImportar.FieldByName('DESCRICAO').AsString;
     ...
     FUsuario.LogUsuario('Tabela de Preços','Na Tabela de preço ['+IntToStr(nTabelaPreco)+'], gerou uma nova grade de Itens (Referencias), clicando no botão "Gerar Tabela"' );
   Finally
   ...
   Try
      ...
      begin
        if bParar then
          Break;
        Application.ProcessMessages;
        vnValorVenda := RetornaReferenciaPrecoVenda(QryListaProduto.FieldByName('REFERENCIA').AsString);
...
begin
  ...
  begin
     QryListaProduto.FieldByName('PRC_VENDA').AsFloat   :=ValorComDesconto(db_ReferenciasImportar.FieldByName('PRC_VENDA').AsFloat,mPercentual);
  end
  else
  if (db_TabelaPreco.FieldByName('AJUSTE_TIPO').AsString='ACRESCIMO') and
     (db_ReferenciasImportar.FieldByName('PRC_VENDA').AsFloat>0) Then
  begin
     QryListaProduto.FieldByName('PRC_VENDA').AsFloat   :=ValorComAcrescimo(db_ReferenciasImportar.FieldByName('PRC_VENDA').AsFloat,mPercentual);
  end
  else
  if (db_TabelaPreco.FieldByName('AJUSTE_TIPO').AsString='NAO') Then
  begin
     QryListaProduto.FieldByName('PRC_VENDA').AsFloat   :=db_ReferenciasImportar.FieldByName('PRC_VENDA').AsFloat;
  end;
end;
...
begin
  ...
  begin
     QryListaProduto.FieldByName('PRC_VENDA').AsFloat   :=ValorComDesconto(QryListaProduto.FieldByName('PRC_VENDA').AsFloat,mPercentual);
  end
  else
  if (db_TabelaPreco.FieldByName('AJUSTE_TIPO').AsString='ACRESCIMO') and
     (QryListaProduto.FieldByName('PRC_VENDA').AsFloat>0) Then
  begin
     QryListaProduto.FieldByName('PRC_VENDA').AsFloat   :=ValorComAcrescimo(QryListaProduto.FieldByName('PRC_VENDA').AsFloat,mPercentual);
  end
  else
  if (db_TabelaPreco.FieldByName('AJUSTE_TIPO').AsString='NAO') Then
  begin
     QryListaProduto.FieldByName('PRC_VENDA').AsFloat   :=QryListaProduto.FieldByName('PRC_VENDA').AsFloat;
  end;
end;
...
begin
   ...
   begin
      QryListaProduto.FieldByName('DESCRICAO').AsString  := fncReferenciaNome(QryListaProduto.FieldByName('REFERENCIA').AsString);
      QryListaProduto.FieldByName('PRC_VENDA').AsFloat   := RetornaReferenciaPrecoVenda(QryListaProduto.FieldByName('REFERENCIA').AsString);
      MontaValorTabela();
   end;
end;
...
begin
   ...
   nTabelaPreco := db_TabelaPreco.FieldByName('CODIGO').AsInteger;
   if bImportando=false then
      FUsuario.LogUsuario('Tabela de Preços','Na Tabela de preço ['+IntToStr(nTabelaPreco)+'], alterou o preço da referencia: ['+QryListaProduto.FieldByName('REFERENCIA').AsString+']' );
end;
...
begin
  ...
  FCodProduto := UpperCase( Trim(QryListaProduto.FieldByName('REFERENCIA').AsString) );
  nTabelaPreco := db_TabelaPreco.FieldByName('CODIGO').AsInteger;
   ...
   begin
     ...
     begin
         If not(QryListaProduto.State in [dsedit]) then
            QryListaProduto.edit;
         QryListaProduto.FieldByName('REFERENCIA').AsString := dbProcurarProduto.ResultFieldAsString('REFERENCIA');
         QryListaProduto.Post;
   ...
   IF Key = VK_F9 Then
   begin
      if not(InputQuery('Localizar REFERÊNCIA','Informe o código da Referência', FCodProduto)) then
         Exit;
