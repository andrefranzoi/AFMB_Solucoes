...
begin
  ...
  //
  //----------------------------------------------------------------------
  ACBrNFe1.Configuracoes.Geral.ForcarGerarTagRejeicao938 := fgtSempre;  // ADD: MAIO/2019

  //----------------------------------------------------------------------
  // Definir tipo de SSL
  //----------------------------------------------------------------------
  SetCertificadoDigital(ACBrNFe1,db_Parametros.FieldByName('SSL_TIPO').AsString);
  SetCertificadoSeguranca(ACBrNFe1,db_Parametros.FieldByName('TSL_TIPO').AsString);
...
//   mValorDesc := 0;
   ...
   inTransportadora  := FrmNotaFiscal.QryVenda.FieldByName('TRANSPORTADORA').AsInteger;
   prcDANFELogo;
   ACBrNFe1.NotasFiscais.Clear;
   SetGerarDANFE();
   SetCertificados();
   ...
   try
     ...
     begin
        ...
        //
        //--------------------------------------------------------------------------
        Ide.natOp         := GetCFOPDescricao( FrmNotaFiscal.QryVenda.FieldByName('CFOP').AsInteger );
        Ide.modelo        := db_Parametros.FieldByName('NFE_MODELO').AsInteger;
        Ide.serie         := db_Parametros.FieldByName('NFE_SERIE').AsInteger;
        Ide.nNF           := NumNFe;
        Ide.cNF           := GerarCodigoDFe(NumNFe); // mudança em junho/2019 (para atender "Nota Técnica 2019/001")
        Ide.dEmi          := FrmNotaFiscal.QryVenda.FieldByName('EMISSAO').AsDateTime;
        Ide.dSaiEnt       := FrmNotaFiscal.QryVenda.FieldByName('DATA_ENTRADASAIDA').AsDateTime;
        Ide.hSaiEnt       := Time;
        ...
        Emit.EnderEmit.CEP        :=  StrToInt( OnlyNumber(db_Parametros.FieldByName('CEP').AsString));
       ...
       ***********************************************************************}
        Emit.CRT := StrToCRT(mRetornoOk, FParametros.Empresa.REGIME_TRIB.ToString);

        if not mRetornoOk then
          Atencao(Format('Código do regime tributário "%d" desconhecido.', [FParametros.Empresa.REGIME_TRIB]));
        ...
        mInscricao     := OnlyNumber( UpperCase(GetClienteCampo(inCliente,'RG_IE')) );
        //---------------------------------------------------------------------------------------------------
        ...
        Dest.EnderDest.fone    := OnlyNumber(Cliente_Endereco(inCliente,'DDD_FONE') + Cliente_Endereco(inCliente,'FONE'));
        Dest.EnderDest.CEP     := StrToInt(OnlyNumber(Cliente_Endereco(inCliente,'CEP')));
        ...
        begin
           mCompraItemFiscal := GetCompraItemFiscal(FrmNotaFiscal.QryVendaItem.FieldByName('IDPRODUTO').AsString);
           with Det.Add do
           begin
              inc(nSequencia);
              Prod.nItem     := nSequencia;
              Prod.cProd     := FrmNotaFiscal.QryVendaItem.FieldByName('IDPRODUTO').AsString;
              Prod.xProd     := FrmNotaFiscal.QryVendaItem.FieldByName('DESCRICAO').AsString;
              Prod.EXTIPI    := '';
             ...
             // vICMSProd  := 0;
              ...
              pICMS      := RoundABNT(FrmNotaFiscal.QryVendaItem.FieldByName('ICMS').AsFloat, 3);
              vICMS      := RoundABNT(FrmNotaFiscal.QryVendaItem.FieldByName('VLR_ICMS').AsFloat, 3);
              mVQtde     := RoundABNT(FrmNotaFiscal.QryVendaItem.FieldByName('QUANTIDADE').AsFloat, 4);
              ...
              begin
                ...
                mVUnitDesc := RoundABNT(FrmNotaFiscal.QryVendaItem.FieldByName('VLR_DESCONTO').AsFloat,3);
              end;
              mVUnit     := RoundABNT(FrmNotaFiscal.QryVendaItem.FieldByName('VLR_UNIT').AsFloat,3);
              mVQtde     := RoundABNT(FrmNotaFiscal.QryVendaItem.FieldByName('QUANTIDADE').AsFloat,4);
              q_TotQTde  := q_TotQTde + mVQtde;

              Prod.IndTot    := itSomaTotalNFe;
              Prod.CFOP      := FrmNotaFiscal.QryVendaItem.FieldByName('CFOP').AsString;
              Prod.uCom      := FrmNotaFiscal.QryVendaItem.FieldByName('UND').AsString;
              Prod.uTrib     := FrmNotaFiscal.QryVendaItem.FieldByName('UND').AsString;
              Prod.qCom      := mVQtde ;
              Prod.qTrib     := mVQtde;
              ...
              // Tributacao
              //-----------------------------------------------------------------------
              Prod.NCM := OnlyNumber(FrmNotaFiscal.QryVendaItem.FieldByName('NCM').AsString);
              Prod.CEST := OnlyNumber(FrmNotaFiscal.QryVendaItem.FieldByName('CEST').AsString);

              mIDOrigemPro := StrToIntDef(GetProdutoCampo(FrmNotaFiscal.QryVendaItem.FieldByName('IDPRODUTO').AsString,'IDCSTORIGEM'),0);
              mIDSTICMS    := StrToIntDef(GetProdutoCampo(FrmNotaFiscal.QryVendaItem.FieldByName('IDPRODUTO').AsString,'IDCSTICMS'),0);
              mIDCSOSNICMS := StrToIntDef(GetProdutoCampo(FrmNotaFiscal.QryVendaItem.FieldByName('IDPRODUTO').AsString,'IDCSOSNICMS'),0);
              mIndEscala   := StrToIntDef(GetProdutoCampo(FrmNotaFiscal.QryVendaItem.FieldByName('IDPRODUTO').AsString,'INDESCALA'),0);
              ...
              begin
                ...
                begin
                  vBC_CRT1_900   := vBC_CRT1_900    + RoundABNT((mVUnit * mVQtde),2);
              ...
              begin
                 ...
                 begin
                    ...
                    begin
                      ...
                      //--------------------------------------------------------------------------------------------------------------------------------------------------------------------//
                      if (TpcnCSTIcms(mIDSTICMS) in [cst00,cst20,cst40,cst41,cst60]) then
                        CST := StrToCSTICMS(LOKResp, FormatFloat('00',mIDSTICMS) );
                    end;
                    ...
                    ICMS.CSOSN     := StrToCSOSNIcms(LOKResp, mIDCSOSNICMS.ToString );
                    ICMS.orig      := StrToOrig(LOKResp, FormatFloat('0',mIDOrigemPro) );;
                    ICMS.modBC     := dbiValorOperacao; //VERIFICAR
                    ICMS.vBC       := 0;
                    ICMS.pICMS     := 0;
                    ICMS.vICMS     := 0;

                    ICMS.pICMS     := RoundABNT(FrmNotaFiscal.QryVendaItem.FieldByName('ICMS').AsFloat, 3);
                    ICMS.vICMS     := RoundABNT(FrmNotaFiscal.QryVendaItem.FieldByName('VLR_ICMS').AsFloat, 3);

                    if FrmNotaFiscal.QryVendaItem.FieldByName('VLR_ICMS').AsFloat > 0 then
                    begin
                    ...
                    begin
                      ...
                      if ((TpcnCSTIcms(mIDSTICMS) in [cst60]) or (ICMS.CSOSN in [csosn500])) then
                      begin
                        ICMS.pST             := PICMS + ICMS.pFCP;

                        if (mCompraItemFiscal <> nil) then
                        begin
                          ICMS.vBCSTRet        := (mCompraItemFiscal.VBCST * mVQtde);
                          ICMS.vICMSSTRet      := (mCompraItemFiscal.vICMSSTRet * mVQtde);
                          ICMS.vICMSSubstituto := (mCompraItemFiscal.VICMS * mVQtde);
                        end;
                      end;
                    end;
                 end; // GRUPO ICMS
...
//
                ...
                begin
                  ...
                  if (FrmNotaFiscal.QryVendaItem.FieldByName('VLR_IPI').AsFloat > 0) then
                  begin
                    vBC    := RoundABNT(mVUnit * mVQtde,3);;
                    pIPI   := FrmNotaFiscal.QryVendaItem.FieldByName('IPI').AsFloat;
                    vIPI   := FrmNotaFiscal.QryVendaItem.FieldByName('VLR_IPI').AsFloat;
                  end;
                end;
                ...
                begin
                  ...
                  if (FrmNotaFiscal.QryVendaItem.FieldByName('VPIS').AsFloat > 0) then
                  begin
                    vBC    := RoundABNT(mVUnit * mVQtde,3);;
                    pPIS   := FrmNotaFiscal.QryVendaItem.FieldByName('PPIS').AsFloat;
                    vPIS   := FrmNotaFiscal.QryVendaItem.FieldByName('VPIS').AsFloat;
                  end;
                end;
                ...
                begin
                  ...
                  if (FrmNotaFiscal.QryVendaItem.FieldByName('VCOFINS').AsFloat > 0) then
                  begin
                    vBC    := RoundABNT(mVUnit * mVQtde,3);;
                    pCOFINS   := FrmNotaFiscal.QryVendaItem.FieldByName('PCOFINS').AsFloat;
                    vCOFINS   := FrmNotaFiscal.QryVendaItem.FieldByName('VCOFINS').AsFloat;
                  end;
                end;
              ...
              //
              //-----------------------------------------------------------------------
              Prod.indEscala := StrToIndEscala(LOKResp,mIndEscala.ToString); // Atenção: Se for "N" (NÃO) é obrigatório informar o CNPJ do fabricante.
              if (Prod.indEscala = ieNaoRelevante) then
              begin
                Prod.CNPJFab := GetCNPJFabricante(FrmNotaFiscal.QryVendaItem.FieldByName('IDPRODUTO').AsString);
              end;
              Prod.cEAN       := GetProdutoCampo(FrmNotaFiscal.QryVendaItem.FieldByName('IDPRODUTO').AsString,'CODIGO_BARRA');;
              Prod.cEANTrib   := GetProdutoCampo(FrmNotaFiscal.QryVendaItem.FieldByName('IDPRODUTO').AsString,'CODIGO_BARRA');;
        ...
        begin
          Total.ICMSTot.vBC    := RoundABNT( FrmNotaFiscal.QryVenda.FieldByName('ICMS_BASECALCULO').AsFloat,3);
          Total.ICMSTot.vICMS  := RoundABNT( FrmNotaFiscal.QryVenda.FieldByName('ICMS_VALOR').AsFloat,3);
        end;
       ...
       ***********************************************************************}
        ...
        case StrToIntDef(FrmNotaFiscal.QryVenda.FieldByName('FRETE').AsString,9) of
        ...
        begin
          Transp.Transporta.CNPJCPF := OnlyNumber( GetClienteCampo(inTransportadora,'CPF_CNPJ') );
          Transp.Transporta.IE :=      OnlyNumber( GetClienteCampo(inTransportadora,'RG_IE') );
          ...
          begin
             qVol := FrmNotaFiscal.QryVenda.FieldByName('FRETE_QUANTIDADE').AsInteger;
             esp := FrmNotaFiscal.QryVenda.FieldByName('FRETE_ESPECIE').AsString;
             marca := FrmNotaFiscal.QryVenda.FieldByName('FRETE_MARCA').AsString;
             nVol := FrmNotaFiscal.QryVenda.FieldByName('FRETE_NUMERO').AsString;
             pesoB := FrmNotaFiscal.QryVenda.FieldByName('FRETE_PBRUTO').AsFloat;
             pesoL := FrmNotaFiscal.QryVenda.FieldByName('FRETE_PLIQUIDO').AsFloat;
          end;
        end;

       Cobr.Fat.nFat  := inNotaFiscal.ToString;
       Cobr.Fat.vOrig := RoundABNT(FrmNotaFiscal.QryVenda.FieldByName('TOTAL_NF').AsFloat,2);
       Cobr.Fat.vDesc := 0;
       Cobr.Fat.vLiq  := RoundABNT(FrmNotaFiscal.QryVenda.FieldByName('TOTAL_NF').AsFloat,2);
       if not (Ide.finNFe in [fnDevolucao, fnAjuste]) then
       begin
         //------------------------------------------------------------------------
         // se houve forma de pagamento
         //------------------------------------------------------------------------
         if FrmNotaFiscal.QryVenda.FieldByName('FORMAPAGTO').AsInteger>0 then
         begin
           QueryCriar(tblVendasFaturas);
           try
             //GetFaturaNFe(tblVendasFaturas,FSistema.Empresa,inCliente,inNotaFiscal);
             GetFaturaNFe(tblVendasFaturas, FSistema.Empresa, FrmNotaFiscal.QryVenda.FieldByName('CODIGO').AsInteger);
             ...
             begin
               ...
               begin
                  ...
                  begin
                     ...
                     vDup      := (tblVendasFaturas.FieldByName('VALOR').AsFloat);
                  end;
             ...
             begin
                ...
                begin
                   ...
                   vPag      := RoundABNT(FrmNotaFiscal.QryVenda.FieldByName('TOTAL_NF').AsFloat,2);
                end;
             end;
           finally
        ...
        InfAdic.infAdFisco := FrmNotaFiscal.QryVenda.FieldByName('OBS2').AsString;
...
begin
  ...
  case ACBrNFe1.Status of
    ...
    stNFeStatusServico: Mensagem := 'Verificando Status do servico...';
    stNFeRecepcao:      Mensagem := 'Enviando dados da NFe...';
    stNfeRetRecepcao:   Mensagem := 'Recebendo dados da NFe...';
    stNfeConsulta:      Mensagem := 'Consultando NFe...';
    stNfeCancelamento:  Mensagem := 'Enviando cancelamento de NFe...';
