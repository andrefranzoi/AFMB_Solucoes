...
uses
  Classe.GetConnect, App.SQL, Classe.Global, Global, Biblioteca,  App.Constantes,
...
begin
  ...
  begin
    mTitulo  := 'Duplicidade';
    mMessage := 'O processo que você está realizando, está tentando duplicar um registro que já existe.';
  end;
  ...
  begin
    ...
    begin
      mTitulo  := 'Exclusão';
      mMessage := 'Não pode ser excluido. Está sendo utilizado por outro registro.';
    end;
...
begin
  Erro('Pasta (ou arquivo) não existe'+
       sLineBreak+
       '"'+aPasta+'"'+
       sLineBreak+
       sLineBreak+
       'Programador, esteja atento a esses detalhes (básicos).'+
       sLineBreak+
       sLineBreak+
       'Dica: Use o instalador do '+_SISTEMA_NOME+' para criar toda estrutura de pastas');
end;
...
Const
  FTabVisivel = 1; // Aqui define qual menu (Ribbon) é ficar visivel primeiro (o padrão é zero - primeiro)
begin
  {14/07/2019: revisado e testado}
  ...
  FDirPrincipal := True; // Indica a principio que o diretorio principal existe.
  FDiasAtivar := 0;
  DBConexao.Connected := False;
  ...
  begin
    FProsseguir := False;
    Erro('[Fechando sistema]'+
          sLineBreak+
          sLineBreak+
         'Motivo: Diretório do sistema não existe.'+
         sLineBreak+
         sLineBreak+
...
begin
  ...
  Try
    ...
    begin
      ...
      dxStatusBar1.Panels[0].Text := Concat(' Usuário: ', FUsuario.Nome+' (Desenvolvedor)');
    end;
    //----------------------------------------------------------------------
...
begin
  {14/07/2019: revisado e testado}
   cfg_EmpresaGravar(FSistema.Empresa);
   dxStatusBar1.Panels[2].Text := Concat('Loja: ', IntToStr(FSistema.Empresa),  '  ', FParametros.Empresa.NomeFantasia ) ;
end;
  ...
  Try
    ...
    Except
      ...
      Erro('[Fechando sistema]'+
           ...
           sLineBreak+
           sLineBreak+
           'Banco de dados exigido: '+mVersaoFB+
           sLineBreak+
           sLineBreak+
           'Sugestão: '+
           sLineBreak+
...
begin
  {14/07/2019: revisado e testado}
   ...
   begin
     ...
     BaseDados.db_Parametros.ParamByName('EMPRESA').AsInteger := FSistema.Empresa;
     BaseDados.db_Parametros.Open;
   end;
   db_empresa.Close;
