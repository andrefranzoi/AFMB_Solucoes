...
begin
   ...
   begin
      ...
      db_ctaPagar.FieldByName('VALOR_SALDO').AsFloat :=
        (db_ctaPagar.FieldByName('VALOR').AsFloat - db_ctaPagar.FieldByName
        ('VALOR_PAGO').AsFloat);
      db_ctaPagar.FieldByName('VALOR_SALDO').AsFloat :=
        (db_ctaPagar.FieldByName('VALOR').AsFloat - db_ctaPagar.FieldByName
        ('VALOR_PAGO').AsFloat);
      db_ctaPagar.FieldByName('HISTORICO').AsString := ' | ' +
        db_ctaPagar.FieldByName('HISTORICO').AsString +
   ...
   begin
      pContasPagarBaixaParcial(StrToIntDef(strFornecedor, 0), Date, Date + 30,
        '0', 'SUBST' + '-[P]', nValorGlobalSobra, '', 0,
...
begin
   ...
   FrmCtaPagarSubstituirChequesInformar :=
     TFrmCtaPagarSubstituirChequesInformar.create(self);
...
begin
   ...
   strFornecedor := IfThen((chkCliente.Checked = false),
   ...
   db_ctaPagar.ParamByName('CL_INICIAL').AsInteger :=
   ...
   lblTotais.Caption := Concat('Selecionados: ', IntToStr(nlRegistros),
     ' | Valor Total: ', FormatSettings.CurrencyString, ' ',
     formatfloat('###,##0.00', nlValor), ' | Total Cheques: ',
     FormatSettings.CurrencyString, ' ', formatfloat('###,##0.00',
     nValorGlobalCheques), ' | Diferença: ', FormatSettings.CurrencyString, ' ',
     formatfloat('###,##0.00', nValorGlobalSobra));
   db_Somar.close;
