...
Type
  ...
  TProduto = Record
    ...
    CodPropFornec         : String; // Codigo do produto no fornecedor (no sistema dele)
    IDFornec              : NativeInt;
    ...
    procedure Limpar;
    procedure GetIDProduto(const aIDProduto: NativeInt); // Codigo proprio do produto (desse sistema)
    function GetByCodFornecedor(const aIDFornecedor: NativeInt; aCodPropFornec: String): Boolean; // Procurar pelo código do produto no fornecedor (no sistema dele)
  end;
...
begin
  if aIDProduto = 0 then
    exit;
  mSQL  := 'SELECT * FROM CAD_PRODUTOS '+Format(' WHERE CODIGO=%d', [aIDProduto]);
  try
    ...
    IDFornec      := mDataSet.FieldByName('FORNECEDOR').AsInteger;       // ID do fornecedor (nesse sistema)
    CodPropFornec := mDataSet.FieldByName('CODIGO_FABRICANTE').AsString; // Codigo do produto no fornecedor (no sistema dele)
    ...
    cBenef      := ''; //mDataSet.FieldByName('CBENEF').AsString;       // Código de Benefício Fiscal utilizado pela UF
    EXTIPI      := mDataSet.FieldByName('EXTIPI').AsString;       // Código EX da TIPI (em caso de serviço, deixar em branco)
    ...
    CFOPSaiInt          := mDataSet.FieldByName('CFOPDENTROESTADO').AsInteger; // CFOP Entrada Interna (dentro do estado)
    CFOPSaiExt          := mDataSet.FieldByName('CFOPFORAESTADO').AsInteger; // CFOP Entrada Externa (fora do estado)
...
function TProduto.GetByCodFornecedor(const aIDFornecedor: NativeInt; aCodPropFornec: String): Boolean;
Var
  mDataSet : TDataSet;
  mSQL : String;
begin
  ...
  if aIDFornecedor = 0 then
    exit;
  mSQL  := 'SELECT * FROM CAD_PRODUTOS '+Format(' WHERE  FORNECEDOR=%d AND CODIGO_FABRICANTE=%s  ', [aIDFornecedor, QuotedStr(aCodPropFornec)]);
  try
    FrmPrincipal.DBConexao.ExecSQL(mSQL, nil, mDataSet);
    if mDataSet.IsEmpty then
      Exit;
