...
uses
   ...
   ISFEdit,
   ISFEditbtn, ISFdbEditbtn, IDBEdit, uEstSearchDialogZeos, Vcl.Mask,
   Vcl.DBCtrls,
   System.StrUtils, ACBrUtil,
   pcnNFe, dxSkinsCore, dxSkinsDefaultPainters, dxDateRanges, ACBrValidador,
...
begin
   if (QryEntrada.IsEmpty) then
      exit;
   if CheckBaixaNFe(QryEntrada.FieldByName('FORNECEDOR').AsInteger,
     StrToIntDef(QryEntrada.FieldByName('NOTAFISCAL').AsString, 0),
     StrToIntDef(QryEntrada.FieldByName('SERIE').AsString, 0)) then
   begin
      Mensagem('Nota Fiscal ' + QryEntrada.FieldByName('NOTAFISCAL').AsString +
        ' já foi importada e baixada.');
      exit;
   end;
   if not pergunta('Confirma o a baixa da compra?') then
      exit;
   spCompraEstoque.ParamByName('NCOMPRA').AsInteger :=
     QryEntrada.FieldByName('CODIGO').AsInteger;
...
begin
   if (QryEntrada.IsEmpty) then
      exit;
   if not CheckBaixaNFe(QryEntrada.FieldByName('FORNECEDOR').AsInteger,
     StrToIntDef(QryEntrada.FieldByName('NOTAFISCAL').AsString, 0),
     StrToIntDef(QryEntrada.FieldByName('SERIE').AsString, 0)) then
   begin
      Mensagem('Nota Fiscal ' + QryEntrada.FieldByName('NOTAFISCAL').AsString +
        ' não foi baixada.' + sLineBreak + 'Não é possivel fazer estorno.');
      exit;
   end;
...
begin
   ...
   begin
      QryEntrada.Close;
      QryEntrada.ParamByName('IDCOMPRA').AsInteger :=
...
begin
   ...
   begin
      ...
      QryFretePagar.FieldByName('FORNECEDOR').AsInteger :=
        FornecedorRec.Transporta.IDTransp; // id transportadora
      QryFretePagar.FieldByName('NOTAFISCAL').AsString :=
        QryEntrada.FieldByName('NOTAFISCAL').AsString;
      QryFretePagar.FieldByName('SERIENF').AsString :=
        QryEntrada.FieldByName('SERIE').AsString;
      QryFretePagar.FieldByName('DATA').AsDateTime :=
        QryEntrada.FieldByName('DATA').AsDateTime;
      QryFretePagar.FieldByName('TIPO').AsString := 'PAGAR';
      QryFretePagar.FieldByName('QUITADO').AsString := 'N';
      QryFretePagar.FieldByName('HISTORICO').AsString :=
        'FRETE/DESPESA (NOTA FISCAL DE COMPRA: ' + QryEntrada.FieldByName
        ('NOTAFISCAL').AsString + ')';
...
begin
   ...
   FornecedorRec.Transporta.IDTransp := QryEntrada.FieldByName('TRANSPORTADORA')
     .AsInteger;
   QryEntrada.ParamByName('IDCOMPRA').AsInteger :=
     QryEntrada.FieldByName('CODIGO').AsInteger;
   QryEntradaItem.ParamByName('IDCOMPRA').AsInteger :=
     QryEntrada.FieldByName('CODIGO').AsInteger;
   QryCtaPagar.ParamByName('IDFORNECEDOR').AsInteger :=
     QryEntrada.FieldByName('FORNECEDOR').AsInteger;
   QryCtaPagar.ParamByName('IDNOTAFISCAL').AsInteger :=
     QryEntrada.FieldByName('NOTAFISCAL').AsInteger;
   QryFretePagar.ParamByName('IDFORNECEDOR').AsInteger :=
     QryEntrada.FieldByName('TRANSPORTADORA').AsInteger;
   QryFretePagar.ParamByName('IDNOTAFISCAL').AsInteger :=
     QryEntrada.FieldByName('NOTAFISCAL').AsInteger;
end;
...
begin
   ...
   if Copy(LowerCase(ACBrNFe1.NotasFiscais.Items[0].NFe.infNFe.ID), 1, 3) <> 'nfe'
   then
   begin
   ...
   try
      ...
      if CheckNFe(FornecedorRec.IDFornec, ACBrNFe1.NotasFiscais.Items[0]
        .NFe.Ide.nNF, ACBrNFe1.NotasFiscais.Items[0].NFe.Ide.serie) then
      begin
      ...
      QryEntrada.FieldByName('MODELO').AsString :=
        IntToStr(ACBrNFe1.NotasFiscais.Items[0].NFe.Ide.modelo);
      QryEntrada.FieldByName('SERIE').AsString :=
        IntToStr(ACBrNFe1.NotasFiscais.Items[0].NFe.Ide.serie);
      QryEntrada.FieldByName('DATA').AsDateTime := ACBrNFe1.NotasFiscais.Items
        [0].NFe.Ide.dEmi;
      QryEntrada.FieldByName('DATAENTRADA').AsDateTime := Date;
      if FornecedorRec.Transporta.IDTransp > 0 then
         QryEntrada.FieldByName('TRANSPORTADORA').AsInteger :=
           FornecedorRec.Transporta.IDTransp;
      QryEntrada.FieldByName('FRETE_VALOR').AsCurrency :=
        ACBrNFe1.NotasFiscais.Items[0].NFe.Total.ICMSTot.vFrete;
      ...
      begin
         ...
         begin
            ...
            begin
               ...
               FProduto.Limpar();
               FProduto.GetByCodFornecedor(FornecedorRec.IDFornec, Prod.cProd);
               // Procura novamente - trazendo dados cadastrados.
            end;

            QryEntradaItem.Append;
            QryEntradaItem.FieldByName('CODIGO').AsInteger :=
              QryEntrada.FieldByName('CODIGO').AsInteger;
            QryEntradaItem.FieldByName('FORNECEDOR').AsInteger :=
              QryEntrada.FieldByName('FORNECEDOR').AsInteger;
            QryEntradaItem.FieldByName('NOTAFISCAL').AsString :=
              IntToStr(ACBrNFe1.NotasFiscais.Items[0].NFe.Ide.nNF);
            QryEntradaItem.FieldByName('MODELO').AsString :=
              IntToStr(ACBrNFe1.NotasFiscais.Items[0].NFe.Ide.modelo);
            QryEntradaItem.FieldByName('SERIE').AsString :=
              IntToStr(ACBrNFe1.NotasFiscais.Items[0].NFe.Ide.serie);
            QryEntradaItem.FieldByName('DATA').AsDateTime :=
              ACBrNFe1.NotasFiscais.Items[0].NFe.Ide.dEmi;
            QryEntradaItem.FieldByName('DATAENTRADA').AsDateTime := Date;

            QryEntradaItem.FieldByName('PRODUTO').AsString :=
              FProduto.CodProprio;
            ...
            QryEntradaItem.FieldByName('VL_TOTAL').AsCurrency :=
              (Prod.vUnCom * Prod.qCom);
            mValFrete :=
              ((QryEntradaItem.FieldByName('VL_UNITARIO').AsCurrency /
              QryEntrada.FieldByName('TOTAL_NF').AsCurrency) *
              QryEntrada.FieldByName('FRETE_VALOR').AsCurrency);
            mRatFrete :=
              ((mValFrete / QryEntradaItem.FieldByName('VL_TOTAL')
              .AsCurrency) * 100);
            ...
            begin
               ...
               QryEntradaItem.FieldByName('ICMS_VALOR').AsCurrency :=
                 ICMS.vICMS;
               ...
               QryEntradaItem.FieldByName('PREDBCST').AsCurrency :=
                 ICMS.PREDBCST;
               ...
               QryEntradaItem.FieldByName('VBCSTRET').AsCurrency :=
                 ICMS.VBCSTRET;
               QryEntradaItem.FieldByName('VICMSSTRET').AsCurrency :=
                 ICMS.VICMSSTRET;
               QryEntradaItem.FieldByName('VICMSSUBSTITUTO').AsCurrency :=
                 ICMS.VICMSSUBSTITUTO;
               QryEntradaItem.FieldByName('PREDBCEFET').AsCurrency :=
                 ICMS.PREDBCEFET;
               QryEntradaItem.FieldByName('VBCEFET').AsCurrency := ICMS.VBCEFET;
               QryEntradaItem.FieldByName('PICMSEFET').AsCurrency :=
                 ICMS.PICMSEFET;
               QryEntradaItem.FieldByName('VICMSEFET').AsCurrency :=
                 ICMS.VICMSEFET;
            end;

            QryEntradaItem.FieldByName('IDNCM').AsString := Prod.NCM;
            QryEntradaItem.FieldByName('IDCEST').AsString := Prod.CEST;
            QryEntradaItem.FieldByName('IDCSTORIGEM').AsInteger :=
              StrToIntDef(OrigToStr(Imposto.ICMS.orig), -1);
            QryEntradaItem.FieldByName('IDCSTICMS').AsInteger :=
              StrToIntDef(CSTICMSToStr(Imposto.ICMS.CST), -1);
            QryEntradaItem.FieldByName('IDCSOSNICMS').AsInteger :=
              StrToIntDef(CSOSNIcmsToStr(Imposto.ICMS.CSOSN), -1);
            QryEntradaItem.FieldByName('IDANP').AsString :=
              IntToStr(Prod.comb.cProdANP);
            QryEntradaItem.FieldByName('IDCSTIPI').AsInteger :=
              StrToIntDef(CSTIPIToStr(Imposto.IPI.CST), -1);
            QryEntradaItem.FieldByName('IDCSTPIS').AsInteger :=
              StrToIntDef(CSTPISToStr(Imposto.PIS.CST), 1 - 1);
            QryEntradaItem.FieldByName('IDCSTCOFINS').AsInteger :=
              StrToIntDef(CSTCOFINSToStr(Imposto.COFINS.CST), -1);
            QryEntradaItem.FieldByName('INDESCALA').AsString :=
              IndEscalaToStr(Prod.indEscala);
         end;
         QryEntradaItem.Post;
         FProduto.Limpar();
      end;
      ...
      begin
         ...
         begin
            ...
            QryCtaPagar.FieldByName('FORNECEDOR').AsInteger :=
              QryEntrada.FieldByName('FORNECEDOR').AsInteger;
            QryCtaPagar.FieldByName('NOTAFISCAL').AsString :=
              QryEntrada.FieldByName('NOTAFISCAL').AsString;
            QryCtaPagar.FieldByName('SERIENF').AsString :=
              QryEntrada.FieldByName('SERIE').AsString;
            QryCtaPagar.FieldByName('DATA').AsDateTime :=
              QryEntrada.FieldByName('DATA').AsDateTime;
            ...
            QryCtaPagar.FieldByName('HISTORICO').AsString :=
              '(NOTA FISCAL DE COMPRA: ' + QryEntrada.FieldByName('NOTAFISCAL')
              .AsString + ')';
      ...
      begin
         ...
         begin
            ...
            QryFretePagar.FieldByName('DOCUMENTO').AsString := 'FRETE-' +
              QryEntrada.FieldByName('NOTAFISCAL').AsString;
            QryFretePagar.FieldByName('TIPO').AsString := 'PAGAR';
            QryFretePagar.FieldByName('VALOR').AsCurrency :=
              ACBrNFe1.NotasFiscais.Items[0].NFe.Total.ICMSTot.vFrete;
            QryFretePagar.FieldByName('VALOR_SALDO').AsCurrency :=
              ACBrNFe1.NotasFiscais.Items[0].NFe.Total.ICMSTot.vFrete;
            QryFretePagar.FieldByName('QUITADO').AsString := 'N';
            QryFretePagar.FieldByName('HISTORICO').AsString :=
              'FRETE/DESPESA (NOTA FISCAL DE COMPRA: ' + QryEntrada.FieldByName
...
begin
   ...
   begin
      QryCtaPagar.FieldByName('EMPRESA').AsInteger := 1;
      QryCtaPagar.FieldByName('FORNECEDOR').AsInteger := FornecedorRec.IDFornec;
      // id transportadora
      QryCtaPagar.FieldByName('NOTAFISCAL').AsString :=
        QryEntrada.FieldByName('NOTAFISCAL').AsString;
      QryCtaPagar.FieldByName('SERIENF').AsString :=
...
begin
   if ((QryEntrada.FieldByName('ORIGEM').AsString = 'XML') or
     (QryEntrada.FieldByName('ATUALIZADO').AsString = 'S')) then
      SetEdicao(false);

   if ((QryEntrada.FieldByName('ORIGEM').AsString <> 'XML') and
     (QryEntrada.FieldByName('ATUALIZADO').AsString = 'N')) then
      SetEdicao(True);
end;
...
begin
   ...
   if rgOpcaoEntrada.ItemIndex = 1 then
   begin
      mNome := fncReferenciaNome(QryEntradaItem.FieldByName('PRODUTO')
        .AsString);
      if (mNome = '') then
      begin
         Mensagem('Produto ' + QryEntradaItem.FieldByName('PRODUTO').AsString +
      ...
      QryEntradaItem.FieldByName('DATA').AsDateTime :=
        QryEntrada.FieldByName('DATA').AsDateTime;
      QryEntradaItem.FieldByName('DATAENTRADA').AsDateTime :=
        QryEntrada.FieldByName('DATAENTRADA').AsDateTime;
   end;
end;
...
begin
   ...
   if rgOpcaoEntrada.ItemIndex = 0 then
      ...
      gbProduto.Caption :=
        '[Produtos] pressione "Insert" para incluir itens na grade';
...
procedure TFrmCompra.DesativarEdicao(aGrid: TcxGridDBTableView;
  aValue: Boolean);
begin
...
begin
   ...
   try
      ...
      mDQuery.ParamByName('CPF_CNPJ').AsString :=
        Formata_CNPJ_CPF(FornecedorRec.CNPJCPF);
      ...
      mDQuery.ParamByName('TRANSPORTADORA').AsInteger :=
        FornecedorRec.Transporta.IDTransp;
      mDQuery.ExecSQL;
