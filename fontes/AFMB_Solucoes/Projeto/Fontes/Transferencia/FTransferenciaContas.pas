...
begin
   ...
   begin
      ...
      db_CtrlBancario.FieldByName('DATA').AsDateTime :=
        db_Transferencias.FieldByName('DATA').AsDateTime;
      db_CtrlBancario.FieldByName('DOCUMENTO').AsString :=
        db_Transferencias.FieldByName('DOCUMENTO').AsString;
      db_CtrlBancario.FieldByName('CTA_CORRENTE').AsInteger :=
        db_Transferencias.FieldByName('CONTABANCOORIGEM').AsInteger;
      db_CtrlBancario.FieldByName('DC').AsString := 'DEBITO';
      db_CtrlBancario.FieldByName('HISTORICO').AsString :=
        'PROVENIENTE DE TRANSFERENCIA - CONTA DESTINO: ' +
        (EditContaCorrenteDestino.Text);
      db_CtrlBancario.FieldByName('VLR_DEBITO').AsFloat :=
        db_Transferencias.FieldByName('VALOR').AsFloat;
      ...
      db_CtrlBancario.FieldByName('DOCUMENTO').AsString :=
        db_Transferencias.FieldByName('DOCUMENTO').AsString;
      db_CtrlBancario.FieldByName('CTA_CORRENTE').AsInteger :=
        db_Transferencias.FieldByName('CONTABANCODESTINO').AsInteger;
      db_CtrlBancario.FieldByName('DC').AsString := 'CREDITO';
      db_CtrlBancario.FieldByName('HISTORICO').AsString :=
        'PROVENIENTE DE TRANSFERENCIA - CONTA ORIGEM: ' +
        (EditContaCorrenteOrigem.Text);
      db_CtrlBancario.FieldByName('VLR_CREDITO').AsFloat :=
        db_Transferencias.FieldByName('VALOR').AsFloat;
      db_CtrlBancario.Post;
      db_CtrlBancario.Close;
   end;
   ...
   begin
      ...
      // primeiro, lançar debito
      db_CAIXA.Append;
      db_CAIXA.FieldByName('DATA').AsDateTime := db_Transferencias.FieldByName
        ('DATA').AsDateTime;
      db_CAIXA.FieldByName('DOCUMENTO').AsString :=
        db_Transferencias.FieldByName('DOCUMENTO').AsString;
      db_CAIXA.FieldByName('CONTACAIXA').AsInteger :=
        db_Transferencias.FieldByName('CONTACAIXAORIGEM').AsInteger;
      db_CAIXA.FieldByName('DC').AsString := 'DEBITO';
      db_CAIXA.FieldByName('HISTORICO').AsString :=
        'PROVENIENTE DE TRANSFERENCIA - CONTA CAIXA DESTINO: ' +
        (EditNomeContaCaixaDestino.Text);
      db_CAIXA.FieldByName('VLR_DEBITO').AsFloat :=
        db_Transferencias.FieldByName('VALOR').AsFloat;
      db_CAIXA.Post;
      db_CAIXA.Close;
      ...
      db_CAIXA.FieldByName('DOCUMENTO').AsString :=
        db_Transferencias.FieldByName('DOCUMENTO').AsString;
      db_CAIXA.FieldByName('CONTACAIXA').AsInteger :=
        db_Transferencias.FieldByName('CONTACAIXADESTINO').AsInteger;
      db_CAIXA.FieldByName('DC').AsString := 'CREDITO';
      db_CAIXA.FieldByName('HISTORICO').AsString :=
        'PROVENIENTE DE TRANSFERENCIA - CONTA CAIXA ORIGEM: ' +
        (EditNomeContaCaixaOrigem.Text);
      db_CAIXA.FieldByName('VLR_CREDITO').AsFloat :=
        db_Transferencias.FieldByName('VALOR').AsFloat;
      db_CAIXA.Post;
      db_CAIXA.Close;
   end;
...
procedure TFrmTransferenciaContas.FormClose(Sender: TObject;
