...
var
   FCodProduto: String;
begin
   FCodProduto := UpperCase(Trim(db_PedidoItens.FieldByName('REFERENCIA')
     .AsString));
   ...
   SP_CalculaPedido.ParamByName('NCODIGO').AsInteger :=
     db_Pedido.FieldByName('CODIGO').AsInteger;
...
begin
   ...
   Result := ((db_Pedido.FieldByName('STATUS').AsString = 'S') or
     (db_Pedido.FieldByName('STATUS').AsString = 'SIM'));
end;
...
begin
   ...
   begin
      ...
      db_PedidoItens.FieldByName('REFERENCIA').AsString :=
        dbProcurarProduto.ResultFieldAsString('REFERENCIA');
      db_PedidoItens.FieldByName('DESCRICAO').AsString :=
        GetProdutoCampo(db_PedidoItens.FieldByName('REFERENCIA').AsString,
        'DESCRICAO');
      GetProduto();
...
begin
   db_Pedido.ParamByName('CODIGO').AsInteger := db_Pedido.FieldByName('CODIGO')
     .AsInteger;
...
begin
   db_PedidoItens.FieldByName('EMPRESA').AsInteger :=
     db_Pedido.FieldByName('EMPRESA').AsInteger;
   db_PedidoItens.FieldByName('CODIGO').AsInteger :=
     db_Pedido.FieldByName('CODIGO').AsInteger;
...
begin
   ...
   if Trim(db_PedidoItens.FieldByName('DESCRICAO').AsString) = '' then
      db_PedidoItens.FieldByName('DESCRICAO').AsString :=
        fncReferenciaNome(FCodProduto);
   if db_PedidoItens.FieldByName('VLR_UNIT').AsFloat <= 0 then
      db_PedidoItens.FieldByName('VLR_UNIT').AsFloat :=
        RetornaReferenciaPrecoVenda(FCodProduto);
   if (db_Pedido.FieldByName('TABELA').AsInteger > 0) then
   begin
      if ValidaReferenciaTabela(db_Pedido.FieldByName('TABELA').AsInteger,
        FCodProduto) then
      begin
         db_PedidoItens.FieldByName('VLR_UNIT').AsFloat :=
           RetornaReferenciaPrecoTabela(db_Pedido.FieldByName('TABELA')
           .AsInteger, db_PedidoItens.FieldByName('REFERENCIA').AsString);
      end;
   end;
end;
...
begin
   ...
   db_Pedido.FieldByName('DATA_ENTREGA').AsDateTime :=
     ...
     (BaseDados.db_Parametros.FieldByName('PED_DIASENTREGA').AsInteger);
   ...
   db_Pedido.FieldByName('FORMAPAGTO').AsInteger :=
     BaseDados.db_Parametros.FieldByName('IDFPAGTOPADRAO').AsInteger;
   db_Pedido.FieldByName('OBSERVACAO').AsString :=
     BaseDados.db_Parametros.FieldByName('PED_MSG').AsString;
...
begin
   ...
   mIDFPagto := 0;
   mIDFPagto := GetClienteFormaPagto(db_Pedido.FieldByName('CLIENTE')
     .AsInteger);
   if mIDFPagto > 0 then
      db_Pedido.FieldByName('FORMAPAGTO').AsInteger := mIDFPagto;
   db_Pedido.FieldByName('VENDEDOR').AsInteger :=
     BuscaClienteVendedor(db_Pedido.FieldByName('CLIENTE').AsInteger);
   db_Pedido.FieldByName('TRANSPORTADORA').AsInteger :=
     BuscaClienteTransportadora(db_Pedido.FieldByName('CLIENTE').AsInteger);
   ...
   if GetClienteCampo(db_Pedido.FieldByName('CLIENTE').AsInteger,
     'OBSERVACAO') > '' Then
      AvisoSistema(GetClienteCampo(db_Pedido.FieldByName('CLIENTE').AsInteger,
        'OBSERVACAO'));
end;
...
begin
   ...
   begin
      ...
      begin
         ...
         AvisoSistema
           ('Pedido não pode ser alterado. Está aprovado para faturamento!' +
           #13 + #10 +
...
begin
   ...
   begin
      ...
      begin
         ...
         AvisoSistema
           ('Pedido não pode ser deletado. Está aprovado para faturamento!' +
           #13 + #10 +
           'Para alterar, é necessário que o responsavel desaprove (reabra) o pedido '
           + 'para que voce possa alterar os dados.');
         exit;
...
begin
   ...
   prcFastReportEmailConfigurar
     (fncBuscaClienteEmail(db_Pedido.FieldByName('CLIENTE').AsInteger));
...
begin
   ...
   begin
      ...
      db_Pedido.ParamByName('CODIGO').AsInteger :=
        dbeditPedidoProcurar.ResultFieldAsInteger('CODIGO');
      db_Pedido.Open;
