...
Const
  ...
  _PASTASISTEMA_DB           = Concat(_DRIVESISTEMA,_SISTEMA_NOME,PathDelim,PASTA_DADOS,PathDelim);
  ...
  _LOGMAIL                   = Concat(_PASTASISTEMA,PathDelim,'Log',PathDelim,'LogMails.log');
  _LOCALINI                  = Concat(_DRIVE,PathDelim,_SISTEMA_NOME,PathDelim,PASTA_BINARIO,PathDelim,CONFIG_INI );

  SITE_EMPRESA               = 'Autor: Nome da sua empresa'; // seu nome ou nome da sua empresa
  ...
  _BANCODADOS_CEP             = _PASTASISTEMA_DB+'basecep'+'.fdb'; // informe aqui se você tiver uma base de dados de CEP
  ...
  _NFE_LOCAL_DANFE         =  Concat(_PASTASISTEMA,PathDelim,'DANFE',PathDelim,_NFE_DANFE);
  _NFE_LOCAL_SCHEMA        =  Concat(_PASTASISTEMA,PathDelim,'SCHEMAS');
  _NFE_LOCAL_PDF           =  Concat(_PASTASISTEMA,PathDelim,'NFE',PathDelim,'PDF');
  _NFE_LOCAL_INUTILIZADAS  =  Concat(_PASTASISTEMA,PathDelim,'NFE',PathDelim,'INUTILIZADAS');
  _NFE_LOCAL_GERADAS       =  CONCAT(_PASTASISTEMA,PathDelim,'NFE',PathDelim,'GERADAS');
  _NFE_LOCAL_EVENTO        =  CONCAT(_PASTASISTEMA,PathDelim,'NFE',PathDelim,'EVENTOS');
  _NFE_LOCAL_SALVAR        =  CONCAT(_PASTASISTEMA,PathDelim,'NFE',PathDelim,'SALVAR');
  _NFCE_LOCAL_GERADAS      =  CONCAT(_PASTASISTEMA,PathDelim,'NFCE',PathDelim,'GERADAS');
  ...
  _BOLETO_DIRLOGO           =  Concat(_PASTASISTEMA,PathDelim,'Boletos',PathDelim,'Logos',PathDelim,'Colorido');
  ...
  // SAT
  //--------------------------------------------------------------------
  _SAT_INTEGRADOR_INPUT   = Concat(_PASTASISTEMA,PathDelim,'Integrador',PathDelim,'Input',PathDelim);
  _SAT_INTEGRADOR_OUTPUT  = Concat(_PASTASISTEMA,PathDelim,'Integrador',PathDelim,'Output',PathDelim);
  _SAT_SCHEMAVENDAAPL     = Concat(_PASTASISTEMA,PathDelim,'Schemas',PathDelim,'CfeDadosVendaAPL_0007.xsd');
  _SAT_SCHEMAVENDASAT     = Concat(_PASTASISTEMA,PathDelim,'Schemas',PathDelim,'CfeDadosVendaSAT_0007.xsd');
  _SAT_LOG                = Concat(_PASTALOG,'SAT_LOG');

  //--------------------------------------------------------------------
  // DLL Sistema
  //--------------------------------------------------------------------
 _DLL_IMAGENS = Concat(_DRIVE,PathDelim,_SISTEMA_NOME,PathDelim,PASTA_BINARIO,PathDelim,_SISTEMA_NOME+'.dll');
