...
uses
  ...
  procedure RegistrarUltimaVenda(TblClientes, Tbl_Pedidos: TFDQuery; CodigoCliente: String);
  ...
  procedure pContasReceberBaixaParcial(ncliente, nVendedor: integer; nData, nVencimento: TDate; nNotafiscal, nDocumento: String; nNovoValor: Real; nPlanoConta: String; nforma_pagto: integer; wAcao: String);
  procedure pContasPagarBaixaParcial(nfornecedor: integer; nData, nVencimento: TDate; nNotafiscal, nDocumento: String; nNovoValor: Real; nPlanoConta: String; nforma_pagto: integer; shitorico, wAcao: String);
  ...
  function GetCFOPProduto(aProduto: Integer; aDentroEstado: Boolean = True): Integer;
  procedure SQLExcluirCtaReceber(aIDVenda: Integer);
  procedure GravarProdutoCaminhoFoto(const IDProduto: Integer; Const aCaminho: String);
...
Begin
   ...
   Begin
     ...
     begin
       ...
       begin
         TblClientes.edit;
         TblClientes.FieldByName('ULTIMA_VENDA').AsDateTime := Tbl_Pedidos.FieldByName('DataVenda').AsDateTime;
         TblClientes.Post;
       end;
     end;
   End;
...
begin
  ...
  try
    ...
    mQuery.ParamByName('historico').AsString    := '(PARCELA COMPLEMENTAR) ' + nDocumento;
...
begin
   ...
   try
     ...
     FParametros.Empresa.DataAtividade                := mQuery.FieldByName('INF_DATABASE').AsDateTime;
     FParametros.Empresa.Nome                         := mQuery.FieldByName('NOME').AsString;
     FParametros.Empresa.NomeFantasia                 := mQuery.FieldByName('NOMEFANTASIA').AsString;
     ...
     FParametros.Empresa.Complemento                  := mQuery.FieldByName('COMPLEMENTO').AsString;
     ...
     FParametros.Empresa.REGIME_TRIB                  := mQuery.FieldByName('REGIME_TRIB').AsInteger;
     FParametros.Empresa.REGIME_TRIBALIQ              := mQuery.FieldByName('REGIME_TRIBALIQUOT').AsFloat;
     FParametros.Financeiro.Juros                     := mQuery.FieldByName('FIN_JUROS').AsFloat;
     FParametros.Financeiro.Cartorio                  := mQuery.FieldByName('FIN_CARTORIO').AsFloat;
     FParametros.Financeiro.CobrancaDias              := mQuery.FieldByName('FIN_COBDIAS').AsInteger;
     FParametros.Financeiro.VendaVistaBaixar          := mQuery.FieldByName('FIN_VENDAVISTABAIXAR').AsString;
     FParametros.Financeiro.DiasBloqueio              := mQuery.FieldByName('FIN_DIASBLOQUEAR').AsInteger;
     FParametros.Financeiro.PagarLancarCaixa          := mQuery.FieldByName('FIN_CTAPAG_LANCCX').AsString;
     FParametros.Financeiro.PagarLancarBanco          := mQuery.FieldByName('FIN_CTAPAG_LANCBCO').AsString;
     FParametros.Financeiro.PagarComissao             := mQuery.FieldByName('FIN_CTAPAG_COMISSAO').AsString;
     FParametros.Financeiro.PagarContaCaixa           := mQuery.FieldByName('FIN_CTAPAG_CONTACX').AsInteger;
     FParametros.Financeiro.PagarPlanoContas          := mQuery.FieldByName('FIN_CTAPAG_PLANOCTA').AsString;
     FParametros.Financeiro.PagarComissaoVencimento   := mQuery.FieldByName('FIN_CTAPAG_COMVECTO').AsInteger;
     FParametros.Financeiro.PagarComissaoContaCaixa   := mQuery.FieldByName('FIN_CTAPAG_COMCTACX').AsInteger;
     FParametros.Financeiro.PagarComissaoCentroCusto  := mQuery.FieldByName('FIN_CTAPAG_COMCTOCUSTO').AsInteger;
     FParametros.Financeiro.PagarComissaoPlanoconta   := mQuery.FieldByName('FIN_CTAPAG_COMPLANCTA').AsString;
     FParametros.Financeiro.ReceberLancarCaixa        := mQuery.FieldByName('FIN_CTAREC_LANCARCAIXA').AsString;
     FParametros.Financeiro.ReceberLancarBanco        := mQuery.FieldByName('FIN_CTAREC_LANCARBANCO').AsString;
     FParametros.Financeiro.ReceberContaCaixa         := mQuery.FieldByName('FIN_CTAREC_CONTACAIXA').AsInteger;
     FParametros.Financeiro.ReceberPlanoContas        := mQuery.FieldByName('FIN_CTAREC_PLANOCONTAS').AsString;
     FParametros.Financeiro.BalcaoLancarCaixa         := mQuery.FieldByName('FIN_BANCAO_LANCCX').AsString;
     FParametros.Financeiro.SiteCotacao               := mQuery.FieldByName('SITECOTACAOMOEDA').AsString;
     FParametros.Compra.PlanoContas                   := mQuery.FieldByName('COMPRA_PLNCTA').AsString;
     FParametros.Compra.CFOP                          := mQuery.FieldByName('COMPRA_CFOP').AsInteger;
     FParametros.Compra.Fornecedor                    := mQuery.FieldByName('COMPRA_IDFORNECEDOR').AsInteger;
     FParametros.Alerta.Estoque                       := mQuery.FieldByName('ALERTA_ESTQ').AsString;
     FParametros.Alerta.Ctareceber                    := mQuery.FieldByName('ALERTA_CTAREC').AsString;
     FParametros.Alerta.CtareceberDias                := mQuery.FieldByName('ALERTA_CTAREC_DIAS').AsInteger;
     FParametros.Alerta.Ctapagar                      := mQuery.FieldByName('ALERTA_CTAPAG').AsString;
     FParametros.Alerta.CtapagarDias                  := mQuery.FieldByName('ALERTA_CTAPAG_DIAS').AsInteger;
     FParametros.Alerta.Pedidos                       := mQuery.FieldByName('ALERTA_PEDIDOS').AsString;
     FParametros.Alerta.Cobranca                      := mQuery.FieldByName('ALERTA_COBRANCA').AsString;
     FParametros.Alerta.Validade                      := mQuery.FieldByName('ALERTA_VALID').AsString;
     FParametros.Alerta.ValidadeDias                  := mQuery.FieldByName('ALERTA_VALID_DIAS').AsInteger;
     FParametros.Alerta.PedidosCompras                := mQuery.FieldByName('ALERTA_PEDCOMPRA').AsString;
     FParametros.CFOP.VendaUFInterno                  := mQuery.FieldByName('CFOP_VNDUFLOCAL').AsInteger;
     FParametros.CFOP.VendaUFExterno                  := mQuery.FieldByName('CFOP_VNDAUFEXTERNO').AsInteger;
     FParametros.CFOP.DevolucaoUFLocal                := mQuery.FieldByName('CFOP_DEVUFINTERNO').AsInteger;
     FParametros.CFOP.DevolucaoUFFora                 := mQuery.FieldByName('CFOP_DEVUFEXTERNO').AsInteger;
     FParametros.CFOP.CompraUFLocal                   := mQuery.FieldByName('CFOP_COMPUFINTERNO').AsInteger;
     FParametros.CFOP.CompraUFFora                    := mQuery.FieldByName('CFOP_COMPUFEXTERNO').AsInteger;
     FParametros.NFE.Ambiente                         := StrToIntDef(mQuery.FieldByName('NFE_AMBIENTE').AsString,0);
     FParametros.NFE.TipoDANFE                        := mQuery.FieldByName('NFE_TIPODANFE').AsString;
     FParametros.NFE.FormaEmissao                     := mQuery.FieldByName('NFE_FORMAEMISSAO').AsString;
     FParametros.NFE.Modelo                           := mQuery.FieldByName('NFE_MODELO').AsString;
     FParametros.NFE.Serie                            := mQuery.FieldByName('NFE_SERIE').AsString;
     FParametros.NFE.Sequencia                        := mQuery.FieldByName('NFE_SEQUENCIA').AsInteger;
     FParametros.NFE.Lote                             := mQuery.FieldByName('NFE_LOTE').AsInteger;
     FParametros.NFE.VersaoNFe                        := Ord(TpcnVersaoDF.ve400);  //mQuery.FieldByName('NFE_VERSAONFE').AsInteger;
     FParametros.NFE.EnviarEmail                      := mQuery.FieldByName('NFE_ENVIAREMAIL').AsString='SIM';
     FParametros.NFE.CertArquivo                      := mQuery.FieldByName('NFE_CERTIFICADO_LOCAL').AsString;
     FParametros.NFE.CertSenha                        := mQuery.FieldByName('NFE_CERTIFICADO_SENHA').AsString;
     FParametros.NFE.CertSerie                        := mQuery.FieldByName('NFE_CERTIFICADO_NUMSERIE').AsString;
     FParametros.NFE.GeradorDANFE                     := 'FORTESREPORT';
     FParametros.Configuracao.LogoPrincipal           := mQuery.FieldByName('INF_IMGPRINCIPAL').AsString;
     FParametros.Configuracao.LogoRelatorios          := mQuery.FieldByName('INF_IMGIMPRIMIR').AsString;
     FParametros.Cliente.Inatividade                  := mQuery.FieldByName('CLI_DIASINATIVAR').AsInteger;
     FParametros.Cliente.Atrasopermitido              := mQuery.FieldByName('CLI_DIASATRASO').AsInteger;
     FParametros.Venda.Vendedor                       := mQuery.FieldByName('VND_IDVENDPADRAO').AsInteger;
     FParametros.Venda.Cliente                        := mQuery.FieldByName('VND_IDCLIENTEPADRAO').AsInteger;
     FParametros.Venda.IDPagto                        := mQuery.FieldByName('IDFPAGTOPADRAO').AsInteger;
     FParametros.Venda.Imprimefatura                  := mQuery.FieldByName('VND_IMPFATURA').AsString;
     FParametros.Pedido.Entrega                       := mQuery.FieldByName('PED_DIASENTREGA').AsInteger;
     FParametros.Pedido.Baixaestoque                  := mQuery.FieldByName('PED_BAIXAESTOQUE').AsString;
     FParametros.Pedido.Mensagem                      := mQuery.FieldByName('PED_MSG').AsWideString;
     FParametros.Email.POPHost                        := Trim(mQuery.FieldByName('EMAIL_POP3HOST').AsString);
     FParametros.Email.POPPorta                       := mQuery.FieldByName('EMAIL_POP3PORTA').AsInteger;
     FParametros.Email.SMTPHost                       := Trim(mQuery.FieldByName('EMAIL_SMTPHOST').AsString);
     FParametros.Email.SMTPPorta                      := mQuery.FieldByName('EMAIL_SMTPPORTA').AsInteger;
     FParametros.Email.Usuario                        := Trim(mQuery.FieldByName('EMAIL_USUARIO').AsString);
     FParametros.Email.Senha                          := Trim(mQuery.FieldByName('EMAIL_SENHA').AsString);
     FParametros.Email.SSLTSL                         := mQuery.FieldByName('EMAIL_SSLTSL').AsString='SIM';
     ...
     FParametros.NFE.RespTec_IDCSRT    := mQuery.FieldByName('RESPTEC_IDCSRT').AsString;
     FParametros.NFE.RespTec_CSRT      := mQuery.FieldByName('RESPTEC_CSRT').AsString;
     FParametros.NFE.RespTec_CNPJ      := mQuery.FieldByName('RESPTEC_CNPJ').AsString;
     FParametros.NFE.RespTec_xContato  := mQuery.FieldByName('RESPTEC_XCONTATO').AsString;
     FParametros.NFE.RespTec_email     := mQuery.FieldByName('RESPTEC_EMAIL').AsString;
     FParametros.NFE.RespTec_Fone      := mQuery.FieldByName('RESPTEC_FONE').AsString;
   finally
     QueryLiberar(mQuery);
   end;
end;
...
begin
   ...
   try
     ...
     mQuery.SQL.Add(' replace(replace(replace(cpf_cnpj, ''.'', ''''), ''/'', ''''), ''-'', '''' ) as  cpf_cnpj ');
...
begin
   ...
   try
     mQuery.SQL.Clear;
     mQuery.SQL.Add(' select * from CAD_CLIENTES where CODIGO='+IntToStr(nCliente));
     mQuery.Open;
...
begin
   ...
   mQuery.Open;
   Result := ((mQuery.FieldByName('BLOQUEADO').AsString='S') or (mQuery.FieldByName('BLOQUEADO').AsString='SIM') );
   finally
     QueryLiberar(mQuery);
   end;
end;
...
begin
   ...
   try
     ...
     mQuery.Open;
     Result := Copy(UpperCase(mQuery.FieldByName('DESCRICAO').AsString),1,60); // limite na NF-e é 60 caracteres
   finally
     QueryLiberar(mQuery);
   end;
end;
...
begin
   ...
   try
     mQuery.SQL.Clear;
     mQuery.SQL.Add(' UPDATE CAD_CLIENTES SET ULTIMAVENDA_DATA=                                    ');
     if (lsOrigem='PEDIDO') then
     begin
        mQuery.SQL.Add(' (SELECT  MAX(PEDIDOS.DATA) AS DATA FROM PEDIDOS                        ');
        mQuery.SQL.Add(' WHERE PEDIDOS.EMPRESA=:NEMPRESA AND PEDIDOS.CLIENTE=:NCLIENTE),  ');
        mQuery.SQL.Add(' ULTIMAVENDA_ORIGEM=:SORIGEM                                                  ');
     end;
     if (lsOrigem='NFE') then
     begin
        mQuery.SQL.Add(' (SELECT  MAX(VENDAS.EMISSAO) AS EMISSAO FROM VENDAS                        ');
        mQuery.SQL.Add(' WHERE VENDAS.EMPRESA=:NEMPRESA AND VENDAS.CLIENTE=:NCLIENTE),  ');
        mQuery.SQL.Add(' ULTIMAVENDA_ORIGEM=:SORIGEM                                                  ');
     end;
     mQuery.SQL.Add(' WHERE                                                                        ');
     mQuery.SQL.Add(' CODIGO=:NCLIENTE                                                            ');
     mQuery.SQL.Add('                                                                              ');
     mQuery.ParamByName('NEMPRESA').AsInteger  :=  FSistema.Empresa;
...
begin
  ...
  try
    ...
    try
      ...
      mQuery.SQL.Add(' WHERE ');
      mQuery.SQL.Add(' DATAENTRADA=(SELECT MAX(DATAENTRADA) FROM COMPRAS_ITENS WHERE PRODUTO=:SPRODUTO)  ');
      mQuery.SQL.Add(' AND  ');
      ...
      mCompraItemFiscal.CodFabricante    := mQuery.FieldByName('PRODUTO').AsString;
      mCompraItemFiscal.Descricao        := mQuery.FieldByName('DESCRICAO').AsString;
      mCompraItemFiscal.QTDE             := mQuery.FieldByName('QUANTIDADE').AsFloat;
      mCompraItemFiscal.PICMS            := mQuery.FieldByName('ICMS').AsFloat;
      mCompraItemFiscal.VICMS            := (mQuery.FieldByName('ICMS_VALOR').AsFloat / mCompraItemFiscal.QTDE);
      mCompraItemFiscal.VBCST            := (mQuery.FieldByName('VBCST').AsFloat / mCompraItemFiscal.QTDE);
      mCompraItemFiscal.PICMSST          := mQuery.FieldByName('PICMSST').AsFloat;
      mCompraItemFiscal.VICMSST          := (mQuery.FieldByName('VICMSST').AsFloat / mCompraItemFiscal.QTDE);
      mCompraItemFiscal.PST              := mQuery.FieldByName('PST').AsFloat;
      mCompraItemFiscal.VBCSTRet         := (mQuery.FieldByName('VBCSTRET').AsFloat / mCompraItemFiscal.QTDE);
      mCompraItemFiscal.VICMSSTRet       := (mQuery.FieldByName('VICMSSTRET').AsFloat / mCompraItemFiscal.QTDE);
      mCompraItemFiscal.VICMSSubstituto  := (mQuery.FieldByName('VICMSSUBSTITUTO').AsFloat / mCompraItemFiscal.QTDE);
      mCompraItemFiscal.PREDBCEfet       := mQuery.FieldByName('PREDBCEFET').AsFloat;
      mCompraItemFiscal.VBCEfet          := (mQuery.FieldByName('VBCEFET').AsFloat / mCompraItemFiscal.QTDE);
      mCompraItemFiscal.PICMSEfet        := mQuery.FieldByName('PICMSEFET').AsFloat;
      mCompraItemFiscal.VICMSEfet        := (mQuery.FieldByName('VICMSEFET').AsFloat / mCompraItemFiscal.QTDE);
...
begin
  ...
  try
    try
      mQuery.Close;
      mQuery.SQL.Clear;
      mQuery.SQL.Add(' SELECT FORNECEDOR, CODIGO_FABRICANTE FROM CAD_PRODUTOS ');
      mQuery.SQL.Add(' WHERE ');
      mQuery.SQL.Add(' CODIGO=:IDCODIGO  ');
      mQuery.ParamByName('IDCODIGO').AsString := aProduto;
      mQuery.Open;
...
begin
   ...
   if mCFOP > 0 then
     Exit(mCFOP); // Se ha CFOP no cadastro do cliente, retorne daqui com o CFOP indicado.
   QueryCriar(mQuery);
   try
     mQuery.SQL.Clear;
...
Var
  mQuery : TFDQuery;
begin
   Result := True; // por padrao é True (empresa e cliente são da mesma UF(estado))
   if aCliente < 1 then
     exit;
   QueryCriar(mQuery);
   try
...
begin
   ...
   mCFOP := StrToIntDef(vCFOP,0); // Por ser NFC-e ou SAT, a venda é presencial (dentro do estado).
   if mCFOP > 0 then
     Exit(mCFOP); // Se ha CFOP no cadastro do produto, retorne daqui com o CFOP indicado.
   if aDentroEstado then
     Result := FParametros.CFOP.VendaUFInterno // cadastrado nas configurações do sistema
   else
     Result := FParametros.CFOP.VendaUFExterno; // cadastrado nas configurações do sistema
end;
...
begin
  ...
  try
    ...
    if (mResultado = '') then // se procurar pela referencia não trouxe resultado, tentar pelo ID do produto
    begin
...
begin
  ...
  try
    ...
    mQuery.SQL.Clear;
    mQuery.SQL.Add(' INSERT INTO CAD_PRODUTOS_FOTOS (CODIGO, FOTO_PATH) VALUES (:CODIGO, :FOTO_PATH) ');
