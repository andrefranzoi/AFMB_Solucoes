...
//    aDBQuery.CopyDataSet(db_Parametros);
    ...
    aDBQuery.FieldByName('EMPRESA').AsVariant := Null; // Enviando "Null" ao banco de dados, forçará usar o Generator com a sequencia correta.
    aDBQuery.FieldByName('NOME').AsString := '<Nova Empresa>';
    aDBQuery.FieldByName('NOMEFANTASIA').AsString := '<Nova Empresa>';
    aDBQuery.Post;
    AvisoSistema('Cadastro foi realizada com sucesso. Foi feito uma cópia da atual empresa selecionada.'+
                 sLineBreak+
                 sLineBreak+
...
begin
  ...
  db_Parametros.close;
  db_Parametros.open;
   AvisoSistema('Proxima vez que abrir o sistema, a imagem da tela principal será alterada para a que você selecionou.');
end;
