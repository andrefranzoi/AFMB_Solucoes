...
begin
   ChecaArquivoXML();
   EnviarEmailNF(db_Vendas_NFE.FieldByName('CODIGO').AsInteger,
     BaseDados.ACBrNFe1, True);
end;
...
begin
   ...
   begin
      AvisoSistemaErro('Arquivo XML não foi encontrado.' + sLineBreak +
        'Arquivo: ' + mArquivo);
      abort;
   end;
end;
...
begin
   ...
   try
      ...
      BaseDados.ACBrNFe1.NotasFiscais.LoadFromFile
        (Trim(db_Vendas_NFE.FieldByName('NFE_ARQUIVO').AsString));
      BaseDados.prcDANFELogo;

      if BaseDados.ACBrNFe1.DANFE.Protocolo = '' then
         BaseDados.ACBrNFe1.DANFE.Protocolo :=
           db_Vendas_NFE.FieldByName('NFE_PROTOCOLOUSO').AsString;

      if BaseDados.ACBrNFe1.WebServices.Retorno.Recibo = '' then
         BaseDados.ACBrNFe1.WebServices.Retorno.Recibo :=
           db_Vendas_NFE.FieldByName('NFE_RECIBO').AsString;

      if BaseDados.ACBrNFe1.WebServices.Retorno.ChaveNFe = '' then
         BaseDados.ACBrNFe1.WebServices.Retorno.ChaveNFe :=
           db_Vendas_NFE.FieldByName('NFE_CHAVEACESSO').AsString;
   ...
   mLogRegistro := 'Imprimiu DANFE da Nota Fiscal Eletrônica ' +
     db_Vendas_NFE.FieldByName('NOTAFISCAL').AsString;
...
begin
   ...
   FrmNotaFiscalSituacaoSEFAZ.MemoDados.Lines.Add
     ('SITUAÇÃO DA NOTA FICAL NA SEFAZ:');
   FrmNotaFiscalSituacaoSEFAZ.MemoDados.Lines.Add('Chave............: ' +
     BaseDados.ACBrNFe1.WebServices.Consulta.NFeChave);
   FrmNotaFiscalSituacaoSEFAZ.MemoDados.Lines.Add('Protocolo........: ' +
     BaseDados.ACBrNFe1.WebServices.Consulta.Protocolo);
   FrmNotaFiscalSituacaoSEFAZ.MemoDados.Lines.Add('Código Retorno...: ' +
     IntToStr(BaseDados.ACBrNFe1.WebServices.Consulta.cStat));
   FrmNotaFiscalSituacaoSEFAZ.MemoDados.Lines.Add('Situação.........: ' +
     BaseDados.ACBrNFe1.WebServices.Consulta.xMotivo);
   FrmNotaFiscalSituacaoSEFAZ.Showmodal;
   mLogRegistro := 'Verificou na SEFAZ situação da Nota Fiscal Eletrônica ' +
     db_Vendas_NFE.FieldByName('NOTAFISCAL').AsString;
   FUsuario.LogUsuario('Processamento de Nota Fiscal Eletrônica', mLogRegistro);
...
begin
   ...
   if strSimNao = '' then
   begin
      FUsuario.LogUsuario('Nota Fiscal | Cancelar',
        'Desistiu de Enviar Carta de Correção para a NF-e ' +
        ZeroEsquerda(db_Vendas_NFE.FieldByName('NOTAFISCAL').AsString, 6) +
        ', após abrir tela que solicita justificativa. ');
      Exit;
   end;
   ...
   begin
      FUsuario.LogUsuario('Nota Fiscal | Cancelar',
        'Desistiu de enviar Carta de Correção para a NF-e ' +
        ZeroEsquerda(db_Vendas_NFE.FieldByName('NOTAFISCAL').AsString, 6) +
        ' após pergunta de confirmação. ');
      Exit;
   end;
   FUsuario.LogUsuario('Nota Fiscal | Cancelar',
     'O sistema perguntou se desejava enviar Carta de Correção para a NF-e ' +
     ZeroEsquerda(db_Vendas_NFE.FieldByName('NOTAFISCAL').AsString, 6));
   try
      MsgEsperar('Carta de Correção NF-e: ' +
        ZeroEsquerda(db_Vendas_NFE.FieldByName('NOTAFISCAL').AsString, 6),
        'Aguarde... Enviando Carta de Correção NF-e.', True);
      ...
      BaseDados.ACBrNFe1.NotasFiscais.Clear;
      BaseDados.ACBrNFe1.NotasFiscais.LoadFromFile
        (Trim(db_Vendas_NFE.FieldByName('NFE_ARQUIVO').AsWideString));
      ...
      begin
         ...
         infEvento.chNFe := BaseDados.ACBrNFe1.NotasFiscais[0]
           .NFe.procNFe.chNFe;
      ...
      begin
         if BaseDados.ACBrNFe1.WebServices.EnvEvento.EventoRetorno.retEvento.
           Items[0].RetInfEvento.cStat <> 135 then
         begin
            FUsuario.LogUsuario('Nota Fiscal | Carta de Correção',
              'Não foi possivel enviar Carta de Correção para NF-e ' +
              ZeroEsquerda(db_Vendas_NFE.FieldByName('NOTAFISCAL').AsString, 6)
              + sLineBreak + sLineBreak +
              'O servidor da SEFAZ retornou a seguinte mensagem: ' + sLineBreak
              + BaseDados.ACBrNFe1.WebServices.EnvEvento.EventoRetorno.
              retEvento.Items[0].RetInfEvento.xMotivo);

            raise Exception.CreateFmt
              ('Ocorreu o seguinte erro enviar a correção da nota fiscal eletrônica:'
              + sLineBreak + 'Código:%d' + sLineBreak + 'Motivo: %s',
              [BaseDados.ACBrNFe1.WebServices.EnvEvento.EventoRetorno.retEvento.
   ...
   // -------------------------------------------------------------------------
   db_Vendas_NFE.Edit;
   db_Vendas_NFE.FieldByName('NFE_SITUACAOSEFAZ').AsWideString :=
     db_Vendas_NFE.FieldByName('NFE_SITUACAOSEFAZ').AsWideString +
   ...
   // ------                                                              -----
   // -------------------------------------------------------------------------
   Aviso('Nota Fiscal ' + ZeroEsquerda(db_Vendas_NFE.FieldByName('NOTAFISCAL')
     .AsString, 6) + ' Carta de Correção enviada com sucesso.');
   mLogRegistro := 'Fez Carta de Correção de NF-e ' +
     ZeroEsquerda(db_Vendas_NFE.FieldByName('NOTAFISCAL').AsString, 6);
   FUsuario.LogUsuario('Processamento de Nota Fiscal Eletrônica', mLogRegistro);
end;
...
begin
   ...
   if pergunta('Deseja enviar essa ' + db_Vendas_NFE.FieldByName('TIPONF')
     .AsString + ' ?') = false then
      Exit;
   try
      MsgEsperar('Enviando NF-e: ' +
        ZeroEsquerda(db_Vendas_NFE.FieldByName('NOTAFISCAL').AsString, 6),
        'Aguarde... transmitindo NF-e', True);
      ...
      try
         db_Vendas_NFE.Edit;
         db_Vendas_NFE.FieldByName('NFE_CERTIFICADO').AsString :=
           BaseDados.ACBrNFe1.Configuracoes.Certificados.NumeroSerie;
         db_Vendas_NFE.FieldByName('NFE_ARQUIVO').AsString :=
           BaseDados.ACBrNFe1.NotasFiscais.Items[0].NomeArq;
         db_Vendas_NFE.FieldByName('NFE_ARQUIVO').AsString :=
           BaseDados.ACBrNFe1.NotasFiscais.Items[0].NomeArq;
         db_Vendas_NFE.FieldByName('NFE_RECIBO').AsString :=
           BaseDados.ACBrNFe1.WebServices.Retorno.Recibo;
         db_Vendas_NFE.FieldByName('NFE_CHAVEACESSO').AsString :=
           BaseDados.ACBrNFe1.WebServices.Retorno.ChaveNFe;
         db_Vendas_NFE.FieldByName('NFE_PROTOCOLOUSO').AsString :=
           BaseDados.ACBrNFe1.WebServices.Retorno.Protocolo;
         db_Vendas_NFE.FieldByName('NFE_SITUACAOCODIGO').AsInteger :=
           BaseDados.ACBrNFe1.WebServices.Retorno.cStat;
         db_Vendas_NFE.FieldByName('NFE_SITUACAOSEFAZ').AsString :=
           BaseDados.ACBrNFe1.WebServices.Retorno.xMotivo;
         db_Vendas_NFE.FieldByName('NFE_ARQUIVO').AsString :=
           BaseDados.ACBrNFe1.NotasFiscais.Items[0].NomeArq;
         ...
         begin
            db_Vendas_NFE.FieldByName('NFE_TRANSMITIDO').AsString :=
              'CONTINGENCIA';
            if UpperCase(db_Vendas_NFE.FieldByName('TIPONF').AsString) = 'NFCE'
            then
               db_Vendas_NFE.FieldByName('NFE_TRANSMITIDO').AsString :=
                 'OFFLINE';
         end;
      ...
      mLogRegistro :=
        'Transmitiu para o servidor da SEFAZ a Nota Fiscal Eletrônica ' +
        db_Vendas_NFE.FieldByName('NOTAFISCAL').AsString;
      FUsuario.LogUsuario('Processamento de Nota Fiscal Eletrônica',
        mLogRegistro);
...
begin
   ...
   begin
      FUsuario.LogUsuario('Nota Fiscal | Cancelar',
        'Desistiu de cancelar a NF-e numero ' +
        ZeroEsquerda(db_Vendas_NFE.FieldByName('NOTAFISCAL').AsString, 6) +
        ', após abrir tela que solicita justificativa. ');
      Exit;
   end;
   ...
   FUsuario.LogUsuario('Nota Fiscal | Cancelar',
     'O sistema perguntou se desejava cancelar a NF-e numero ' +
     ZeroEsquerda(db_Vendas_NFE.FieldByName('NOTAFISCAL').AsString, 6));
   try
      MsgEsperar('Cancelar NF-e: ' +
        ZeroEsquerda(db_Vendas_NFE.FieldByName('NOTAFISCAL').AsString, 6),
        'Aguarde... cancelando NF-e.', True);
      ...
      begin
         ...
         infEvento.chNFe := BaseDados.ACBrNFe1.NotasFiscais[0]
           .NFe.procNFe.chNFe;
         infEvento.detEvento.nProt := BaseDados.ACBrNFe1.NotasFiscais[0]
           .NFe.procNFe.nProt;
      ...
      begin
         ...
         begin
            FUsuario.LogUsuario('Nota Fiscal | Cancelar',
              'Não foi possivel cancelar NF-e numero ' +
              ZeroEsquerda(db_Vendas_NFE.FieldByName('NOTAFISCAL').AsString, 6)
              + sLineBreak + sLineBreak +
              'O servidor da SEFAZ retornou a seguinte mensagem: ' + sLineBreak
              + BaseDados.ACBrNFe1.WebServices.EnvEvento.EventoRetorno.
         ...
         db_Vendas_NFE.FieldByName('CANCELAMENTO_USUARIO').AsWideString :=
           FUsuario.Nome;
         db_Vendas_NFE.FieldByName('CANCELAMENTO_CODIGORETORNO').AsInteger :=
           BaseDados.ACBrNFe1.WebServices.EnvEvento.EventoRetorno.retEvento.
           Items[0].RetInfEvento.cStat;
         db_Vendas_NFE.FieldByName('CANCELAMENTO_PROTOCOLO').AsWideString :=
           BaseDados.ACBrNFe1.WebServices.EnvEvento.EventoRetorno.retEvento.
           Items[0].RetInfEvento.nProt;
         db_Vendas_NFE.FieldByName('CANCELAMENTO_SITUACAO').AsWideString :=
           BaseDados.ACBrNFe1.WebServices.EnvEvento.EventoRetorno.retEvento.
           Items[0].RetInfEvento.xMotivo;
         db_Vendas_NFE.FieldByName('NFE_SITUACAOSEFAZ').AsWideString :=
           'Cancelada na SEFAZ';
         ...
         SPNFE_PROCESSAR.ParamByName('NVENDA').AsInteger :=
           db_Vendas_NFE.FieldByName('CODIGO').AsInteger;
   ...
   // -------------------------------------------------------------------------
   Aviso('Nota Fiscal ' + ZeroEsquerda(db_Vendas_NFE.FieldByName('NOTAFISCAL')
     .AsString, 6) + ' cancelada com sucesso.');
   SituacaoNFE();

   // -------------------------------------------------------------------------
   // ------                                                              -----
   // -------------------------------------------------------------------------
   mLogRegistro := 'Cancelou a Nota Fiscal Eletrônica ' +
     ZeroEsquerda(db_Vendas_NFE.FieldByName('NOTAFISCAL').AsString, 6);
   FUsuario.LogUsuario('Processamento de Nota Fiscal Eletrônica', mLogRegistro);
...
begin
   ...
   begin
      FUsuario.LogUsuario('Nota Fiscal | Cancelar',
        'Desistiu de Inutilizar NF-e de "' + ZeroEsquerda(mNFInicial.ToString,
        6) + ' a ' + ZeroEsquerda(mNFFinal.ToString, 6) +
        '", após abrir tela que solicita justificativa. ');
      Exit;
   end;
   ...
   if pergunta('Tem certeza que deseja Enviar Carta de Correção?') = false then
   begin
      FUsuario.LogUsuario('Nota Fiscal | Inutilizar',
        'Desistiu de Inutilizar NF-e de "' + ZeroEsquerda(mNFInicial.ToString,
        6) + ' a ' + ZeroEsquerda(mNFFinal.ToString, 6) +
        '", após pergunta de confirmação. ');
      Exit;
   ...
   FUsuario.LogUsuario('Nota Fiscal | Inutilizar',
     'O sistema perguntou se desejava Inutilizar NF-e de "' +
     ZeroEsquerda(mNFInicial.ToString, 6) + ' a ' +
     ZeroEsquerda(mNFFinal.ToString, 6) + '"');

   try
      MsgEsperar('Inutilizar de Correção NF-e de "' +
        ZeroEsquerda(mNFInicial.ToString, 6) + ' a ' +
        ZeroEsquerda(mNFFinal.ToString, 6) + '"',
        'Aguarde... Enviando Inutilização.', True);
      ...
      BaseDados.ACBrNFe1.WebServices.Inutiliza(FParametros.Empresa.CNPJ,
        strJustificativa, mAno, 55, FParametros.NFe.Serie.ToInteger, mNFInicial,
        mNFFinal);
      ...
      begin
         ...
         db_Vendas_NFE.FieldByName('CANCELAMENTO_PROTOCOLO').AsWideString :=
           BaseDados.ACBrNFe1.WebServices.Inutilizacao.Protocolo;
         db_Vendas_NFE.FieldByName('NFE_SITUACAOSEFAZ').AsWideString :=
      ...
      begin
         raise EDatabaseError.CreateFmt('Código:%d' + sLineBreak + 'Motivo: %s',
           [BaseDados.ACBrNFe1.WebServices.Inutilizacao.cStat,
   ...
   mLogRegistro := 'Inutilizou a NF-e ' +
     ZeroEsquerda(db_Vendas_NFE.FieldByName('NOTAFISCAL').AsString, 6);
   FUsuario.LogUsuario('Processamento de Nota Fiscal Eletrônica', mLogRegistro);
end;
...
procedure TFrmNotaFiscalEmitidas.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
