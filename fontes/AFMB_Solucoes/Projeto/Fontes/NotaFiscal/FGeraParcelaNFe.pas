...
begin
   ...
   db_FormaPagto.ParamByName('FORMAPAGTO').AsInteger :=
     FrmNotaFiscal.QryVenda.FieldByName('FORMAPAGTO').AsInteger;
   db_FormaPagto.open;
   { verificar saldo }
   ...
   Begin
      Atencao('É necessário informar a forma de pagamento');
      exit;
   End;

   If (FrmNotaFiscal.QryVenda.FieldByName('CLIENTE').AsInteger < 1) then
   Begin
      Atencao('É necessário informar um cliente');
      exit;
   End;
...
begin
   ...
   If qt_parc <= 0 then
   begin
      Atencao(' Quantidades de parcelas abaixo de 1. Não é possivel prosseguir.'
   ...
   begin
      ...
      begin
         Atencao('Não atingiu parcela minima exigida pelo sistema. ' +
   ...
   begin
      ...
      if db_FormaPagto.FieldByName('DIAS').AsInteger = 0 then
         dtVencimento := FrmNotaFiscal.QryVenda.FieldByName('EMISSAO')
           .asDateTime;
      ...
      QryCtaReceber.FieldByName('CLIENTE').AsInteger :=
        FrmNotaFiscal.QryVenda.FieldByName('CLIENTE').AsInteger;
      QryCtaReceber.FieldByName('NOTAFISCAL').AsInteger :=
        FrmNotaFiscal.QryVenda.FieldByName('NOTAFISCAL').AsInteger;
      QryCtaReceber.FieldByName('DATA').asDateTime :=
        FrmNotaFiscal.QryVenda.FieldByName('EMISSAO').asDateTime;
      ...
      QryCtaReceber.FieldByName('VENDEDOR').AsInteger :=
        FrmNotaFiscal.QryVenda.FieldByName('VENDEDOR').AsInteger;
      QryCtaReceber.FieldByName('PARCELA').AsInteger := parc_atu;
      QryCtaReceber.FieldByName('DOCUMENTO').AsString :=
        FrmNotaFiscal.QryVenda.FieldByName('CODIGO').AsString + '/' +
...
procedure TFrmGeraParcelaNFe.DBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TFrmGeraParcelaNFe.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
begin
   ...
   Begin
      Atencao('Não há parcelas geradas.');
      exit; // sair
   End;
   ModalResult := mrOk;
end;
