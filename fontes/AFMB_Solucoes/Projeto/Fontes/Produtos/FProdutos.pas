...
begin
   ...
   if FStart then
   begin
     LockWindowUpdate(Handle); // Evitar refresh de tela (para evitar que formulario fique piscando na tela)
     SendMessage(Handle, WM_SETREDRAW, WPARAM(False), 0); // Evitar refresh de tela (para evitar que formulario fique piscando na tela)
   end;
   FIDProduto := 0;
   FIDFornecedor := 0;
...
begin
  if (QryProduto.state in [dsEdit, dsInsert]) then
    QryProduto.FieldByName('IDCEST').AsString := GetNCMCEST(QryProduto.FieldByName('IDNCM').AsString);
end;
...
begin
  ...
  TFloatField(QryProduto.FieldByName('ESTOQUEMINIMO')).DisplayFormat := '##0.##0';
  TFloatField(QryProduto.FieldByName('ESTOQUEMAXIMO')).DisplayFormat := '##0.##0';
  TFloatField(QryProduto.FieldByName('ESTOQUESALDO')).DisplayFormat := '##0.##0';
...
begin
   ...
   QryProduto.ParamByName('IDCODIGO').AsInteger := QryProduto.FieldByName('CODIGO').AsInteger;
  ...
  begin
     ...
     begin
       try
         QryUpdateCodProprio.ParamByName('EMPRESA').AsInteger := FSistema.Empresa;
         QryUpdateCodProprio.ParamByName('IDCODIGO').AsInteger := QryProduto.FieldByName('CODIGO').AsInteger;
...
begin
   ...
   begin
      QryProduto.FieldByName('PRC_CUSTO').AsCurrency := QryProduto.FieldByName('PRC_VENDA').AsCurrency;
   end;
   QryProduto.FieldByName('PRC_VENDA').AsCurrency := RoundABNT(QryProduto.FieldByName('PRC_VENDA').AsCurrency,2);
...
begin
  QryProdutoFoto.FieldByName('CODIGO').AsInteger :=  QryProduto.FieldByName('CODIGO').AsInteger;
  QryProdutoFoto.FieldByName('REFERENCIA').AsString :=  QryProduto.FieldByName('REFERENCIA').AsString;
end;
...
begin
   ...
   if QryProduto.FieldByName('PRC_VENDA').AsCurrency < 0 then
     QryProduto.FieldByName('PRC_VENDA').AsCurrency := QryProduto.FieldByName('PRC_CUSTO').AsCurrency;
   if QryProduto.FieldByName('PRC_VENDA').AsCurrency < QryProduto.FieldByName('PRC_CUSTO').AsCurrency then
     QryProduto.FieldByName('PRC_VENDA').AsCurrency := QryProduto.FieldByName('PRC_CUSTO').AsCurrency;
...
begin
  ...
  db_ChecarExistencia.ParamByName('REFERENCIA').AsString := QryProduto.FieldByName('REFERENCIA').AsString;
  ...
  begin
    ...
    Begin
       ...
       Begin
          ...
          Informar('Produto ' + QryProduto.FieldByName('REFERENCIA').AsString + ' já está cadastrado');
...
begin
  try
    ...
    Begin
      ...
      FotoReferencia.Picture.LoadFromFile(QryProdutoFoto.FieldByName('FOTO_PATH').AsString);
    End
    Else
    Begin
       FotoReferencia.Picture := Nil;
...
begin
  ...
  if FIDProduto > 0 then
    Aviso('Código do Produto: '+IntToStr(FIDProduto)+ ' | '+mCodProdProprio);
  QryProduto.Close;
  ModalResult := mrOk;
end;
...
begin
  ...
  begin
    ...
    //
    //----------------------------------------------------------------------
    QryProduto.FieldByName('REFERENCIA').AsString          := OnlyNumber(varProduto.CodPropFornec);
    QryProduto.FieldByName('CODIGO_FABRICANTE').AsString   := varProduto.CodPropFornec;
    ...
    //----------------------------------------------------------------------
    varProduto.Limpar();
    QryProduto.FieldByName('PRC_VENDA').AsCurrency := 0; // Forçar ficar 0 (zero) para chamar atenção do usuário.
  end;
  LockWindowUpdate(0); // Evitar refresh de tela (para evitar que formulario fique piscando na tela)
