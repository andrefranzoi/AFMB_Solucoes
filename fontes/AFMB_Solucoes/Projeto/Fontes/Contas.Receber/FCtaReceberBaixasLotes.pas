...
var
   ...
   nlValor, nValorGlobalBaixar, nValorGlobalCheques, nValorGlobalSobra: Real;
...
begin
   ...
   SPFIN_CTARECEBER_BAIXARLOTE.ParamByName('NEMPRESA').AsInteger :=
     FSistema.Empresa;
   ...
   SPFIN_CTARECEBER_BAIXARLOTE.ParamByName('NCLIENTE').AsInteger :=
     StrToIntDef(Trim(EditCliente.Text), 0);
   SPFIN_CTARECEBER_BAIXARLOTE.ParamByName('NMOTIVOBAIXA').AsInteger :=
     StrToIntDef(strMotivoBaixa, 0);
   SPFIN_CTARECEBER_BAIXARLOTE.ParamByName('NCONTACORRENTE').AsInteger :=
     StrToIntDef(strContaCorrente, 0);
   SPFIN_CTARECEBER_BAIXARLOTE.ParamByName('NCONTACAIXA').AsInteger :=
     StrToIntDef(strContaCaixa, 0);
   SPFIN_CTARECEBER_BAIXARLOTE.ParamByName('STIPO').AsString :=
   ...
   begin
      pContasReceberBaixaParcial(StrToIntDef(Trim(EditCliente.Text), 0),
        StrToIntDef(GetClienteCampo(StrToIntDef(Trim(EditCliente.Text), 0),
        'VENDEDOR'), 0), Date, Date + 30, '0', 'BAIXALOTE', nValorGlobalSobra,
        '', 0, '');
   end;
...
begin
   ...
   db_ctaReceber.ParamByName('CL_INICIAL').AsInteger :=
     StrToIntDef(Trim(EditCliente.Text), 0);
   ...
   db_Somar.ParamByName('CL_INICIAL').AsInteger :=
     StrToIntDef(Trim(EditCliente.Text), 0);
   ...
   lblTotais.Caption := Concat('Titulos: ', IntToStr(nlRegistros),
...
begin
   prcParametros;
   FrmCtaReceberBaixasLotesSelecionar :=
     TFrmCtaReceberBaixasLotesSelecionar.Create(self);
...
begin
   ...
   FrmCtaReceberBaixasLotesCheques :=
     TFrmCtaReceberBaixasLotesCheques.Create(self);
