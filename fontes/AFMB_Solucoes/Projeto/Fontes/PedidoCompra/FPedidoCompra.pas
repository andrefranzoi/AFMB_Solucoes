...
begin
   ...
   TFloatField(db_PedidoCompra.FieldByName('QTDE_PEDIDA')).DisplayFormat :=
     '###,##0';
end;

procedure TFrmPedidoCompra.db_PedidoCompraAfterPost(DataSet: TDataSet);
begin
   db_PedidoCompra.ParamByName('CODIGO').AsInteger :=
     db_PedidoCompra.FieldByName('CODIGO').AsInteger;
end;
...
begin
   ...
   begin
      AvisoSistema
        ('Você informou uma [Transportadora], informe tambem o campo [Frete por conta] ');
      abort;
   end;
...
begin
   ...
   TFloatField(db_PedidoCompraItens.FieldByName('QTDE_PEDIDA'))
     .DisplayFormat := '';
   TFloatField(db_PedidoCompraItens.FieldByName('QTDE_PEDIDA'))
     .EditFormat := '';
end;
...
begin
   db_PedidoCompraItens.FieldByName('EMPRESA').AsInteger :=
     db_PedidoCompra.FieldByName('EMPRESA').AsInteger;
   db_PedidoCompraItens.FieldByName('CODIGO').AsInteger :=
     db_PedidoCompra.FieldByName('CODIGO').AsInteger;
   GetDetProduto();
end;
...
begin
   ...
   if Trim(db_PedidoCompraItens.FieldByName('DESCRICAO').AsString) = '' then
      db_PedidoCompraItens.FieldByName('DESCRICAO').AsString :=
        GetProdutoCampo(db_PedidoCompraItens.FieldByName('PRODUTO').AsString,
        'DESCRICAO');
   if Trim(db_PedidoCompraItens.FieldByName('UND').AsString) = '' then
      db_PedidoCompraItens.FieldByName('UND').AsString :=
        GetProdutoCampo(db_PedidoCompraItens.FieldByName('PRODUTO').AsString,
        'UNIDADE');
   if db_PedidoCompraItens.FieldByName('VALOR_UNIT').AsFloat <= 0 then
      db_PedidoCompraItens.FieldByName('VALOR_UNIT').AsFloat :=
        GetProdutoPrecoCusto(db_PedidoCompraItens.FieldByName('PRODUTO')
        .AsString);
end;
...
begin
   ...
   SPPEDIDOCOMPRA_CALCULAR.ParamByName('CODIGO').AsInteger :=
     db_PedidoCompra.FieldByName('CODIGO').AsInteger;
...
begin
   ...
   try
      ViewPedidoCompra.ParamByName('CODIGO').AsInteger :=
        db_PedidoCompra.FieldByName('CODIGO').AsInteger;
      ViewPedidoCompraItens.ParamByName('CODIGO').AsInteger :=
        db_PedidoCompra.FieldByName('CODIGO').AsInteger;
...
begin
   ...
   begin
      ...
      db_PedidoCompra.close;
      db_PedidoCompra.ParamByName('CODIGO').AsInteger :=
        dbProcurarPedido.ResultFieldAsInteger('CODIGO');
      db_PedidoCompra.Open;
      db_PedidoCompraItens.close;
      db_PedidoCompraItens.Open;
   end;
