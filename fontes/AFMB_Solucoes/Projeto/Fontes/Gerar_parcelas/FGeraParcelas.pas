...
begin
   db_FormaPagto.close;
   db_FormaPagto.ParamByName('FORMAPAGTO').AsInteger := FrmVendaExpressa.qryVenda.FieldByName('FORMAPAGTO').AsInteger;
   db_FormaPagto.open;
   { verificar saldo  }
...
begin
   ...
   begin
      ...
      INC(parc_atu);
      dtVencimento := FrmVendaExpressa.qryVenda.FieldByName('EMISSAO').asDateTime + db_FormaPagto.FieldByName('DIAS').asInteger;
      if db_FormaPagto.FieldByName('DIAS').asInteger = 0 then
         dtVencimento := FrmVendaExpressa.qryVenda.FieldByName('EMISSAO').asDateTime;
      ...
      QryCtaReceber.FieldByName('IDVENDA').AsInteger    := FrmVendaExpressa.qryVenda.FieldByName('CODIGO').AsInteger;
      QryCtaReceber.FieldByName('CLIENTE').AsInteger    := FrmVendaExpressa.qryVenda.FieldByName('CLIENTE').AsInteger;
      QryCtaReceber.FieldByName('DATA').asDateTime := FrmVendaExpressa.qryVenda.FieldByName('EMISSAO').asDateTime;
      ...
      QryCtaReceber.FieldByName('FORMA_PAGTO').AsInteger := FrmVendaExpressa.qryVenda.FieldByName('FORMAPAGTO').AsInteger;
      QryCtaReceber.FieldByName('VENDEDOR').AsInteger := FrmVendaExpressa.qryVenda.FieldByName('VENDEDOR').AsInteger;
      QryCtaReceber.FieldByName('PARCELA').AsInteger  := parc_atu;
      QryCtaReceber.FieldByName('DOCUMENTO').AsString := FrmVendaExpressa.qryVenda.FieldByName('CODIGO').AsString + '/' + ZeroEsquerda(IntToStr(parc_atu),3);
...
begin
  ...
  vnCreditoSaldo := ChecarCreditoSaldo(FrmVendaExpressa.qryVenda.FieldByName('CLIENTE').AsInteger);
...
begin
  ...
  begin
    ...
    if mIDCliente > 0 then
      FrmVendaExpressa.qryVenda.FieldByName('CLIENTE').AsInteger := mIDCliente;
    if mIDFPagto > 0 then
      FrmVendaExpressa.qryVenda.FieldByName('FORMAPAGTO').AsInteger := mIDFPagto;
    If (FrmVendaExpressa.qryVenda.state in [dsEdit, dsInsert]) then
      FrmVendaExpressa.qryVenda.Post;
  end;
end;
