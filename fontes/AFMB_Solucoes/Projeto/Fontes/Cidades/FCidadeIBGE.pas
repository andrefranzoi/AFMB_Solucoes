...
type
   TFrmCidadeIBGE = class(TForm)
      ...
      procedure pgControlChanging(Sender: TObject;
...
begin
   ...
   if (TVarData(FCodigoRegistro).VType = varString) and (FCodigoRegistro <> '')
   then
   begin
   ...
   if (TVarData(FCodigoRegistro).VType = varInteger) and (FCodigoRegistro > 0)
   then
   begin
      dbCidadeIBGE.Locate('CEP', FCodigoRegistro, []);
      FCodigoRegistro := 0;
