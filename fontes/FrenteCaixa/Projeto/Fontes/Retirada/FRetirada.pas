...
type
  TFrmRetirada = class(TFrmTemplateForm)
    QryUsuario: TFDQuery;
    EditID: TLabeledEdit;
...
procedure TFrmRetirada.EditValorKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmRetirada.EditValorKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TFrmRetirada.GetUsuario;
begin
...
procedure TFrmRetirada.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
  if Key=VK_F2 then
...
procedure TFrmRetirada.btnOkClick(Sender: TObject);
var
  mTexto   : String;
  mData    : TDateTime;
  mValor   : Currency;
begin
  ...
  //
  //--------------------------------------------------------------------
  SQLInsertRetirada(FrmPrincipal.QryMovimento.FieldByName('ID').AsInteger,
  ...
  begin
    mTexto := 'Movimento do dia: '+FormatDateTime('c',mData )+sLineBreak+
              ...
              'Valor da Retirada do Caixa: R$ '+FormatFloat('###,##0.00',mValor)+
              ' '+sLineBreak+
              ' '+sLineBreak+
              ' '+sLineBreak+
  ...
  ImprimirDiversos('RETIRADA DO CAIXA',mTexto);
  ModalResult := mrOk;
