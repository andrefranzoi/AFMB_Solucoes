...
begin
   ...
   Begin
      Informar('É necessário informar a forma de pagamento','Contas a Receber',True);
      Exit;
   End;

   If (FrmPrincipal.QryVenda.FieldByName('CLIENTE').AsInteger < 1) then
   Begin
      Informar('É necessário informar um cliente','Contas a Receber',True);
      Exit;
   End;
...
begin
   ...
   Begin
      Informar('Não há parcelas geradas.','Contas a Receber',True);
      Exit; // sair
   End;
  ModalResult := mrOk;
end;
